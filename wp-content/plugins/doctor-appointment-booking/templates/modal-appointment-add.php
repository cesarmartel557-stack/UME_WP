...
<div class="row mt-4">
	<div class="col">
		<form method="post" class="appointment-add-form"
            ...
            <div class="form-group">
                ...
                <select name="chamber_id" class="form-control select2" id="chamber_id"
                        style="width: 100%;" onchange="load_schedules()">
					<?php
					$chambers = \Ctmdcd\api\DataApi::get_chambers();
                    $default_chamber = \Ctmdcd\api\DataApi::get_settings( 'default_chamber_id' );
					foreach ( $chambers as $row ):
                        if ( $row->status == 0 )
                            continue;
						?>
                        <option value="<?php echo $row->chamber_id; ?>"
                            <?php if ( $default_chamber == $row->chamber_id ) echo 'selected'; ?>>
							<?php echo $row->name; ?>
                        </option>
					<?php endforeach; ?>
                </select>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control datepicker" name="appointment_timestamp" id="timestamp"
                    value="<?php echo date( get_option( 'date_format' ) );?>">
            </div>
...
<script>
    ...
    function validate() {
        var chamber_id = jQuery('#chamber_id').val();
        date = jQuery('#timestamp').val();
        var schedule = jQuery('#schedule').val();
        if (chamber_id == '' || date == '' || schedule == '') {
            notify( 'You must select a chamber, date and schedule', 'warning' );
            return false;
        }
        if ( patient_type === 'old' ) {
    ...
    function showResponse() {
        ...
        make_ajax_call( '<?php echo wp_create_nonce( 'ctmdcd-ajax-nonce' ); ?>', ajaxurl, 'section-appointment-list', 'appointment-list', date );

        make_ajax_call( '<?php echo wp_create_nonce( 'ctmdcd-ajax-nonce' ); ?>', ajaxurl, 'section-appointment-calendar', 'appointment-calendar' );
    ...
    function load_schedules() {
        ...
        make_ajax_call( '<?php echo wp_create_nonce( 'ctmdcd-ajax-nonce' ); ?>', ajaxurl, 'section-appointment-schedule-selector', 'schedule-holder', day, chamber_id, false );
    }
