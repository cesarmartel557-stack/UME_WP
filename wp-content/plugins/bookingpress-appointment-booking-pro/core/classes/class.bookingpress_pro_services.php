...
if ( ! class_exists( 'bookingpress_pro_services' ) ) {
	class bookingpress_pro_services Extends BookingPress_Core {
		function __construct() {
			...
			add_action( 'bookingpress_modify_default_holidays', array( $this, 'bookingpress_modify_default_holidays_with_service_daysoff'), 5.5, 4 );
		...
		*/
		...
		function bookingpress_add_service_dynamic_vue_methods_func() {
			...
			show_edit_dayoff_div(day_off_id, currentElement) {
				...
				vm.service_dayoff_arr.forEach(function(item, index, arr){
					if (item.id == day_off_id) {
						...
						vm.service_dayoff_form.dayoff_repeate_times = item.repeat_times;
						vm.service_dayoff_form.dayoff_repeate_date = item.repeat_date;
					}
			...
			close_service_dayoff() {
				...
				vm.service_dayoff_form.dayoff_repeate_times = '3';
				vm.service_dayoff_form.dayoff_repeate_date = '<?php esc_html_e( date( 'Y-m-d', strtotime( '+1 year') ) ); ?>';
