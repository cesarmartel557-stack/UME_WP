...
final class CRB_Activity {
		...
		if ( ! empty( $alert_list ) ) {
			...
			foreach ( $alert_list as $hash => $alert ) {
				...
				if ( ! empty( $alert['6'] ) ) {
					...
					$msg[] = sprintf( __( 'Search string: %s', 'wp-cerber' ), $alert['6'] );
					$msg_masked[] = sprintf( __( 'Search string: %s', 'wp-cerber' ), $alert['6'] );
				}
	 ...
	 * @since 9.6.6.11
	 */
	public static function get_rows( string $query = '', ?int &$num = null ): array {
