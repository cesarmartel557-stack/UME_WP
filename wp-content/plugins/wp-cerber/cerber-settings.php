...
/**
 * Used to store plugin settings with WP Options API.
 * Retired since 9.3.4, leaved for migration purpose.
 *
 * @see CRB_INPUT_NAME_PREFIX
 */
...
/**
 * A list of retired WP options - for migration and maintenance purposes.
 */
const CRB_LEGACY_OPTIONS = [
	CERBER_OPT,
	CERBER_OPT_H,
	CERBER_OPT_U,
	CERBER_OPT_A,
...
// Consolidated settings stored here, since 9.3.4
const CERBER_CONFIG = 'cerber_configuration';
// Geo-IP settings, stored separately
const CERBER_GEO_RULES = 'geo_rule_set';
// Processed WP Cerber settings, stored in the sets
...
/**
 * A set of Cerber settings stored in the WP Options table
 *
 * @param bool $all
 *
 * @return array
...
function cerber_select( $name, $list, $selected = null, $class = '', $id = '', $multiple = false, $placeholder = '', $data = array(), $atts = '' ) {
	...
	foreach ( $list as $key => $value ) {
		...
		$options[] = '<option value="' . $key . '" ' . $s . '>' . crb_escape_html( $value ) . '</option>';
	}
 ...
 *
 * @param string $setting If specified returns the default value for this setting
 * @param bool $resolve Include values that requires resolving via external code
 *
 * @return array|mixed|null
 *
 * @see CRB_Settings_Registry for setting fields definitions
 * @see CRB_INPUT_NAME_PREFIX
 */
function cerber_get_defaults( string $setting = '', $resolve = true ) {
	...
	if ( $resolve ) {
		$default_values[ CERBER_OPT_U ]['authonlymsg'] = __( 'Only registered and logged in users are allowed to view this website', 'wp-cerber' );
		$default_values[ CERBER_OPT_OS ]['ds_4opts_list'] = CRB_DS::get_settings_list( false );
	}

	if ( $setting ) {
		foreach ( $default_values as $section ) {
...
function cerber_upgrade_settings( $ver = '' ) {
	...
	$setting_defs = CRB_Settings_Registry::get_setting_fields();
	$upgrade_from = array();
	$obsolete = array();
	...
	if ( ! $key = get_site_option( LAB_SITE_KEY_DATA ) ) {
		$key = cerber_get_site_option( LAB_SITE_KEY_DATA );
	}

	if ( $key ) {
		if ( cerber_update_set( LAB_SITE_KEY_DATA, $key ) ) { // new
...
function crb_get_settings( $setting_id = '', $purge_cache = false, $use_defaults = true ) {
	...
	if ( defined( 'CERBER_WP_OPTIONS' ) && CERBER_WP_OPTIONS ) {
		...
		// A missing or corrupted option is normalized silently: this branch has no static
		// cache and runs on every call, so registering an issue here is not affordable
	...
	if ( ! isset( $cache ) || $purge_cache ) {
		...
		// By design, any empty (falsy) stored value means the settings do not exist: it is
		// never unserialized and never treated as data corruption. Only a non-empty stored
		// value must unserialize to an array; anything else is a sign of data corruption.

		$stored_settings = $settings_raw ? crb_unserialize( $settings_raw ) : false;

		// A corrupted (non-unserializable) DB value triggers an automatic restoration
		// attempt; default settings are used as the last resort instead of crashing
		// on array operations below
		...
		if ( is_array( $stored_settings ) ) {
			...
			if ( $settings_raw ) {
				...
				// Data corruption detected: attempting to restore the settings from the
				// automatic backup copy. Both the successful and the failed restoration
				// are reported via CRB_Issues inside recover().

				if ( $restored_settings = CRB_Settings_Backup::recover( strlen( $settings_raw ) ) ) {
					$cache = $restored_settings;
				}
			}

			// Not restored: fall back to the default settings
		...
		if ( ( $cf = crb_array_get( $addons, 'cloudflare' ) ) && is_array( $cf ) ) {
 ...
 */
function crb_build_email_recipients( $type = '', $args = array() ): array {

	// A ready-to-use list of recipients
	...
	if ( $list = $args['user_list'] ?? false ) {
		foreach ( $list as $user_id ) {
			if ( $u = crb_get_userdata( $user_id ) ) {
				$name = crb_sanitize_mail_display_name( $u->display_name );
				$emails[] = $name ? $name . ' <' . $u->user_email . '>' : $u->user_email;
			}
		}
	}

	if ( ! $emails ) { // Fallback to general notification email
		$emails = (array) crb_get_settings( 'email' );
	}

	if ( ! $emails ) { // Fallback to site admin email
		$emails = get_site_option( 'admin_email' );
		$emails = array( $emails );
	}

	if ( $type == 'plugin_activated' ) { // Plugin activation alert
		if ( is_super_admin() ) {
...
function cerber_cloud_sync( $data = array() ) {
	...
	if ( $quick || $full ) {
		$set = array(
			...
			crb_build_email_recipients( 'scan' )
		);
...
/**
 * Retrieve saved role policies for the given role
 *
 * @param string $role_id
 *
 * @return array Role policies
 */
function cerber_get_role_policies( $role_id ) {
	static $cache;
	...
	$role_conf = crb_array_get( $conf, $role_id, array() );
