 ...
 *
 */
final class CRB_Traffic_Log_Screen {

	/** @var array<string,mixed> Render arguments for this screen render pass. */
	private array $args;

	/** Whether toolbar and search controls should be rendered. */
	private bool $ui_controls_enabled;

	/** Whether Traffic Inspector logging is enabled for this render pass. */
	private bool $logging_enabled;

	/** Whether the current request applies Traffic Log search or filter criteria. */
	private bool $search_active;

	/** Whether the Traffic Log table has no stored entries. */
	private bool $traffic_log_empty;

	/**
	 * Initializes request-local Traffic Log screen state.
	 *
	 * @param array<string,mixed> $args Screen rendering arguments.
	 *
	 * @since 9.8.1
	 */
	private function __construct( array $args = array() ) {
		$this->args = $args;
		$this->ui_controls_enabled = empty( $args['compact_view'] );
		$this->logging_enabled = (bool) crb_get_settings( 'timode' );
		$this->search_active = (bool) array_intersect_key( CRB_Traffic_Log::QUERY_PARAMS, crb_get_query_params() );
		$this->traffic_log_empty = CRB_Traffic_Log::is_empty();
	}

	/**
	 * Renders and outputs the HTTP Traffic Log admin screen.
	 *
	 * @param array<string,mixed> $args Optional arguments to control UI, filtering, and row limits.
	 ...
	 * @since 9.8.1
	 */
	public static function render_screen( array $args = array(), $echo = true ) {

		$screen = new self( $args );
		$result_html = $screen->render();
	...
	/**
	 * Renders HTTP Traffic Log admin screen.
	 *
	...
	private function render() {
		...
		$fetched = CRB_Traffic_Log::fetch_rows( $this->args );

		$filters = $fetched->get_element( 'filters', array() );
		$traffic_log_rows = $fetched->get_element( 'rows', array() );
		$total_rows = (int) $fetched->get_element( 'total', 0 );

		if ( $this->ui_controls_enabled ) {
			$toolbar_html = self::render_toolbar( $this->traffic_log_empty, (int) crb_get_settings( 'tithreshold' ), (bool) $total_rows );
			$info_blocks_html = self::render_info_blocks( $filters['ip'], $filters['user'] );
		...
		elseif ( $traffic_log_rows ) {
			// Fetch and prepare all related data
			...
			$table_html = self::render_log_table( $traffic_log_rows, $enriched_data, $grouped_events, $this->args );

			// Add pagination
			if ( $this->ui_controls_enabled ) {
				$table_html .= cerber_page_navi( $total_rows, $filters['per_page'] );
			}

			$result_html .= $table_html;

			if ( ! $this->logging_enabled ) {
				$result_html = '<p class="cerber-margin" style="padding: 0.5em; font-weight: 600;">' . __( 'Logging is currently disabled, you are viewing historical information.', 'wp-cerber' ) . '</p>' . $result_html;
			}
		}
		else {
			// Render the "no results" message
			$result_html .= $this->render_empty_state();
		}
	...
	private static function render_toolbar( $is_log_empty, $threshold, $has_results ) {
		...
		if ( ! $is_log_empty ) {
			...
			$filter_links = crb_build_quick_nav_block( $nav_links, 'traffic' );
		...
		return '<div id="crb-admin-toolbar-secondary-row"><div><div style="display: table-cell; width: 80%;">' . $filter_links . '</div><div style="display: table-cell;">' . $refresh_button . '</div></div><div>' . $right_actions . '</div></div><br style="clear: both;">';
	}
	...
	private static function render_table_row( $log_entry, $enriched_data, $events, $row_class, $args ) {
		...
		// User Agent cell
		$ua_cell = crb_escape_html( CRB_User_Agent_Parser::detect( $log_entry->request_details[1] ?? '' ) );
	...
	/**
	 * Renders the UI for when no log entries are found using request-local screen state.
	 *
	...
	private function render_empty_state() {
		...
		if ( $this->search_active ) {
			...
			if ( ! $this->traffic_log_empty ) {
				$ui_hints[] = crb_ui_element( 'p', array(), crb_ui_link( $base_url, __( 'View all logged requests', 'wp-cerber' ) ) );
			}
		}
		else {
		...
		if ( ! $this->logging_enabled ) {
			...
			if ( ! $this->search_active ) {
				$ui_hints[] = crb_ui_element( 'p', array(), crb_ui_link( 'https://wpcerber.com/wordpress-traffic-logging/', __( 'Documentation', 'wp-cerber' ), array( 'target' => '_blank' ) ) );
			}
		}

		// Self-repair if the log table we think is missing or corrupted
		if ( $this->traffic_log_empty ) {
			cerber_watchdog( true );
		}

		return crb_ui_renderer()->render_element( crb_ui_message_box( $ui_hints ) );
	}
