...
add_action( 'admin_init', function () {
	...
	if ( ! $last
        ...
        CRB_Deferred_Tasks::add( 'cerber_do_hourly_2' );
        CRB_Deferred_Tasks::add( 'cerber_daily_run' );
	}
        ...
        <div id="crb-scan-tiles">
            ...
            <div class="scan-tile">
                <div><p><span id="crb-scanned-files" data-init="0">0</span> / <span id="crb-total-files"
                    ...
                    <p><?php echo __( 'Scanned / Files to scan', 'wp-cerber' ); ?></p>
                </div>
            </div>
	...
	if ( $scan = cerber_get_scan() ) {
		if ( ! $scan['finished'] ) {
			if ( $scan['cloud']
				...
				/* translators: %s is the date and time when the previous scan started. */
...
function cerber_ref_upload_form() {
    ...
    <div id="crb-ref-upload-dialog" style="display: none;">
        ...
        <?php /* translators: %s is the maximum file size allowed for upload. */ ?>
...
function cerber_manual_scan() {
	...
	if ( cerber_is_http_post() && $scan_do = crb_get_post_fields( 'cerber_scan_do' ) ) {
		...
		$scanner = crb_scan_run_scanner( $scan_do, $mode );
...
add_action( 'wp_ajax_cerber_view_file', function () {
	...
	if ( ! @file_exists( $file_name ) ) {
		crb_admin_stop_ajax( 'Error: The requested file doesn\'t exist or has been deleted: ' . crb_escape_html( $file_name ) . '. Please run a new malware scan.' );

		return;
	}

	if ( ! @is_file( $file_name ) ) {
		crb_admin_stop_ajax( 'Error: The requested file is not an ordinary file: ' . crb_escape_html( $file_name ) );
	...
	$the_file = cerber_db_get_row( 'SELECT * FROM ' . cerber_get_db_prefix() . CRB_SCANFILES_TABLE . ' WHERE scan_id = ' . $scan_id . ' AND file_name = "' . $file_name . '"' );
...
add_action( 'wp_ajax_cerber_scan_bulk_files', function () {
	...
	foreach ( $post['files'] as $file_name ) {
		...
		$the_file = cerber_db_get_row( 'SELECT * FROM ' . cerber_get_db_prefix() . CRB_SCANFILES_TABLE . ' WHERE scan_id = ' . $scan_id . ' AND file_name = "' . $file_name . '"', MYSQL_FETCH_OBJECT );
	...
	crb_scan_diagnostic_log( $errors );
...
function cerber_show_quarantine() {
	...
	$first = ( crb_get_page_num() - 1 ) * $per_page;
	$last = $first + $per_page;
	...
	if ( ! $list ) {
		if ( ! $filter_scan ) {
			echo crb_ui_renderer()->render_element( crb_ui_message_box( array( __( 'There are no files in the quarantine at the moment.', 'wp-cerber' ) ) ) );
		}
		else {
			echo __( 'No files match the specified filter.', 'wp-cerber' ) . ' <a href="' . crb_admin_link_for_html( 'scan_quarantine' ) . '">' . __( 'Click here to see the full list of files', 'wp-cerber' ) . '</a>.';
		}
...
function cerber_show_ignore() {
	...
	if ( ! $list = cerber_get_set( 'ignore-list' ) ) {
		echo crb_ui_renderer()->render_element( crb_ui_message_box( array( __( 'The list is empty.', 'wp-cerber' ) ) ) );
...
/**
 * Renders the message box shown when no scan data is available for analytics reports.
 *
 * @return string The rendered message box HTML.
 */
function crb_scan_no_message() {
	return crb_ui_renderer()->render_element( crb_ui_message_box( array(
		crb_ui_element( 'p', array(), crb_ui_element( 'b', array(), __( 'No data for generating reports', 'wp-cerber' ) ) ),
...
function cerber_show_files( $ext, $scan_id ) {
	...
	$files = cerber_db_get_results( 'SELECT SQL_CALC_FOUND_ROWS file_name, file_mtime, file_size FROM ' . cerber_get_db_prefix() . CRB_SCANFILES_TABLE . ' WHERE scan_id = ' . $scan_id . ' AND file_ext = "' . $ext . '" ' . $limit );
	...
	echo '<div style="display: table-cell"><h3>' . $title . '</h3></div><div style="display: table-cell; vertical-align: middle;">  &nbsp; <a href="' . crb_admin_link_for_html( 'scan_insights' ) . '#crb_ins_ext_list" class="page-title-action">' . __( 'Back to list', 'wp-cerber' ) . '</a></div>';
...
function crb_scan_insights_exts( $scan_id ) {
	...
	$base = crb_admin_link_for_html( 'scan_insights' );
...
function crb_scan_insights_lrgst( $scan_id ) {
	...
	$files_revalt = warp_run_db( static function ( CRB_Database $db ) use ( $scan_id ): Revalt {
		return $db->table( cerber_get_db_prefix() . CRB_SCANFILES_TABLE )
		          ->select( 'file_name', 'file_mtime', 'file_size' )
	...
	if ( $files_revalt->has_errors() ) {
		return crb_scan_no_message();
	}

	$files = $files_revalt->get_results( array() );
...
function crb_scan_insights_duplicates( $scan_id ) {
	...
	// SQL Doesn't work
	/* ( ! $files = cerber_db_get_results( 'SELECT file_name, file_mtime, file_size, file_hash FROM ' . cerber_get_db_prefix() . CRB_SCANFILES_TABLE . ' WHERE scan_id = ' . $scan_id . ' GROUP BY file_hash HAVING COUNT(*) > 1 LIMIT 100' ) ) {
		return '';
	}*/

	if ( ! $files = cerber_db_get_col( 'SELECT file_hash FROM ' . cerber_get_db_prefix() . CRB_SCANFILES_TABLE . ' WHERE scan_id = ' . $scan_id . ' AND file_size !=0 AND file_hash !="" ' ) ) {
		return crb_scan_no_message();
	}
	...
	$list = array();
	foreach ( $dup as $hash ) {
		if ( $fl = cerber_db_get_results( 'SELECT file_name, file_mtime, file_size FROM ' . cerber_get_db_prefix() . CRB_SCANFILES_TABLE . ' WHERE scan_id = ' . $scan_id . ' AND file_hash = "' . $hash . '"' ) ) {
...
function crb_scan_last_full() {
	...
	$scans = cerber_db_get_col( 'SELECT DISTINCT scan_id FROM ' . cerber_get_db_prefix() . CRB_SCANFILES_TABLE . ' WHERE scan_mode = 1 ORDER BY scan_id DESC LIMIT 1' );
...
function cerber_make_table( array $rows, array $heading = array(), string $id = '', string $class = '', string $body_class = '', string $head_class = '', $show_footer = true ) {
	...
	foreach ( $rows as $row ) {
		...
		foreach ( $row as $cell ) {
			...
			if ( is_array( $cell ) ) {
				$cell_class = crb_escape_html( $cell['class'] ?? '' );
				$val        = $cell['cell'] ?? 'none';
			}
            else {
				$cell_class = '';
	...
	if ( $heading ) {
		...
		$head_class = crb_escape_html( $head_class );
	...
	$id = ( $id ) ? 'id="' . crb_escape_html( $id ) . '"' : '';

	return '<table ' . $id . ' class="widefat crb-table cerber-margin ' . crb_escape_html( $class ) . '">' . $thead . $tfoot . '<tbody class="' . crb_escape_html( $body_class ) . '"><tr>' . implode( '</tr><tr>', $tr ) . '</tr></tbody></table>';
...
function crb_show_admin_announcement( $text = '', $big = true ) {
    ...
    echo '<div class="cerber-msg crb-announcement crb-dismiss-scope' . ' ' . $class . '"><table><tr><td id="crb-ann-logo"></td>' .
...
/**
 * Streams the HTTP request traffic log as a CSV download and terminates the current request.
 *
 * Builds an unbounded traffic export from the supplied admin filter arguments and writes the CSV
 * response headers, metadata, column headings, and every matching row to the output stream. The
 * accepted filters are the traffic log query arguments listed in CRB_Traffic_Log::QUERY_PARAMS,
 * the same filters used by the traffic admin screen; any filter absent from $params falls back to the
 * current request's query parameters.
 *
 * @param array<string,mixed> $params Optional traffic log filter arguments; supplied values take
 *                                    precedence over the current request's query parameters.
 *
 * @return void This function does not return to the caller.
 */
function cerber_export_traffic( $params = array() ) {

	crb_try_raise_php_limits( '512M' );

	// Keep the stream projection aligned with the CSV mapping below.
	$args = is_array( $params ) ? $params : array();
	$args['columns'] = array( 'ip', 'stamp', 'uri', 'session_id', 'user_id', 'processing', 'request_method', 'http_code', 'wp_type', 'blog_id' );
	...
	// Delay header output until export setup is known to be valid.

	$send_header = static function ( int $total, array $range ) {

		$info = array();

		if ( $range ) {
			$info[] = array( 'Newest record:', _crb_csv_date( $range['newest'] ) );
			$info[] = array( 'Oldest record:', _crb_csv_date( $range['oldest'] ) );
		}
	...
	// Resolve query metadata before output so setup errors can still use wp_die().

	$export = CRB_Traffic_Log::stream_log( $args );

	if ( $export->has_errors() ) {

		$error_message = __( 'WP Cerber: unable to export the traffic log.', 'wp-cerber' );

		// Show internal diagnostics only to administrators.

		if ( current_user_can( 'manage_options' ) ) {
			$root_cause = $export->get_root_cause();

			if ( $root_cause ) {
				$error_message .= $root_cause['message'] . ' [ERROR CODE: ' . $root_cause['code'] . ']';
			}
		}

		wp_die( crb_escape_html( $error_message ) );
	}

	$total  = (int) $export->get_element( 'total', 0 );
	$range  = $export->get_element( 'range', array() );
	$stream = $export->get_element( 'stream', array() );

	// Make the stream handle the only remaining owner of the unbuffered result.
	$export = null;

	$first = true;

	// Always release the unbuffered result, including future early exits.
	try {
		foreach ( $stream as $row ) {

			if ( $first ) {
				$send_header( $total, $range );
				$first = false;
			}
		...
		// Trigger generator cleanup before sending the final response terminator.
		$stream = null;
	}
...
/**
 * Streams the activity log as a CSV download and terminates the current request.
 *
 * Builds an unbounded activity export from the supplied admin filter arguments, writes the CSV
 * response headers, metadata, column headings, and every matching row to the output stream. The
 * accepted filters are the same activity log query arguments handled by CRB_Activity::stream_log();
 * non-array input is ignored and treated as an empty filter set. Dates honor the plain_date setting.
 *
 * On export setup failure no CSV bytes are sent; the request is terminated with wp_die() and an
 * escaped diagnostic message. On success the function always emits a CSV file, including a
 * header-only file when no rows match, releases the unbuffered database stream, and exits.
 *
 * @param array<string,mixed> $params Optional activity log filter arguments.
 *
 * @return void This function does not return to the caller.
 */
function cerber_export_activity( $params = array() ) {
	...
	// Preserve the validated admin filters for the export stream.
	$args = is_array( $params ) ? $params : array();
	...
	$send_header = static function ( int $total, array $filters, array $range ) {

		$info = array();

		if ( $filters['ip'] ) {
			$info[] = array( 'Filter by IP:', $filters['ip'] );
		}

		if ( $filters['user_id'] ) {
			$user      = crb_get_userdata( $filters['user_id'] );
			$user_name = $user ? $user->display_name : '';
			$info[]    = array( 'Filter by user:', $user_name );
		}

		if ( $filters['search'] ) {
			$info[] = array( 'Search results for:', $filters['search'] );
		}
	...
	$export = CRB_Activity::stream_log( $args );

	if ( $export->has_errors() ) {

		$export_message = __( 'WP Cerber: unable to export the activity log.', 'wp-cerber' );
		...
		if ( current_user_can( 'manage_options' ) ) {
			...
			if ( $root_cause ) {
				$export_message .= ' [' . $root_cause['code'] . '] ' . $root_cause['message'];
			}
		}

		wp_die( crb_escape_html( $export_message ) );
	}

	$total   = (int) $export->get_element( 'total', 0 );
	$filters = $export->get_element( 'filters', array() );
	$range   = $export->get_element( 'range', array() );
	$stream  = $export->get_element( 'stream', array() );
	...
	try {
		foreach ( $stream as $row ) {
			...
			if ( $first ) {
				$send_header( $total, $filters, $range );
				$first = false;
			}
...
/**
 * Sends HTTP headers and outputs the initial metadata and column headings for a CSV export.
 *
 * Generated metadata, additional info rows, and headings are all escaped by
 * cerber_send_csv_line(). The output uses CRLF line endings to preserve the
 * historical CSV export format.
 *
 * @param string $file_name_prefix Prefix for the downloaded file.
 * @param int $total Total number of rows expected.
 * @param array<int, string> $heading Optional list of column headings.
 * @param array<int, array<int, string>> $additional_info Optional list of metadata rows, each row being a list of cell values.
 *
 * @return void
 */
function cerber_send_csv_header( string $file_name_prefix, int $total, array $heading = array(), array $additional_info = array() ) {
	$file_name = $file_name_prefix . '-' . crb_site_label() . '.csv';

	crb_send_file_headers( $file_name, 'text/csv', true );

    // Common header info

	cerber_send_csv_line( array( 'Generated by:', 'WP Cerber Security ' . CERBER_VER ) );
	cerber_send_csv_line( array( 'Website:', crb_get_blogname_decoded() ) );
	cerber_send_csv_line( array( 'Date:', cerber_date( time(), false ) ) );
	cerber_send_csv_line( array( 'Rows in this file:', (string) $total ) );

	// Additional header info

	foreach ( $additional_info as $info_row ) {
		cerber_send_csv_line( $info_row );
	}
...
/**
 * Sends HTTP headers for a database export download (tables or settings).
 * Optionally disables output buffering to allow direct streaming for massive tables.
 *
 * @param string $file_name The name of the file to be downloaded.
 * @param string $ct Content-Type header value.
 * @param bool $disable_buffering Set to true ONLY for streaming large datasets via Generator.
 */
function crb_send_file_headers( string $file_name, string $ct = 'text/csv', bool $disable_buffering = false ): void {
	$file_name = rawurlencode( $file_name );

	if ( $disable_buffering ) {
		// Aggressively destroy all buffer layers
		while ( ob_get_level() > 0 ) {
			ob_end_clean();
		}
		// Bypass Nginx FastCGI buffering
		header( 'X-Accel-Buffering: no' );
	}
	else {
		// Safely clear only the topmost buffer to remove any garbage (e.g., BOM)
		if ( ob_get_level() > 0 ) {
			ob_clean();
		}
	}
	...
	// Prevent caching of sensitive DB dumps
