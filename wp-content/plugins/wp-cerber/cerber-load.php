...
function crb_sessions_kill( $tokens, $user_id = null, $admin = true ) {
	...
	if ( $admin ) {
		...
		if ( $total ) {
			...
			if ( $total === 1 ) {
				$msg = __( 'User session has been terminated', 'wp-cerber' );
			}
			else {
				/* translators: %d is the number of terminated user sessions. */
				$msg = sprintf( _n( '%d user session has been terminated', '%d user sessions have been terminated', $total, 'wp-cerber' ), $total );
			}
		...
		if ( defined( 'CONCATENATE_SCRIPTS' ) ) {
			...
			if ( CONCATENATE_SCRIPTS ) {
				if ( is_admin() || nexus_is_valid_request() ) {
					CRB_Issues::add( 'conscripts', 'The PHP constant CONCATENATE_SCRIPTS is already defined somewhere else', array( 'setting_id' => 'adminphp' ) );
				}
			}
		}
...
/**
 * Check ACLs for given IP.
 *
 * @param string $ip IP address
 * @param string $tag 'W' to check against Allowed ACL, 'B' to check against Blocked ACL.
 * @param int $acl_slice The ACL ID, default is 0 = global ACL.
 * @param object|false $row If the given IP is in an ACL, it will be populated with an appropriate ACL entry row object:
 *                         for IPv4 includes all columns
 *                         for IPv6 includes comments column only
 *
 * @return bool|string Returns 'W' if the IP in the Allowed ACL, 'B' if the IP is in the Blocked ACL,
 *                     false if the IP is not in an ACL specified by $acl_slice
 */
...
/**
 * Checks whether an IPv6 address belongs to the provided IPv6 range.
 *
 * @param string $ip The IPv6 address
 * @param array $range The IP range
 *
 * @return bool
 */
function crb_ipv6_is_in_range( string $ip, array $range ) {
...
/**
 * Checks whether IPv6 numeric chunks are within any range from the provided range list.
 *
 * The IPv6 address is represented by the second and third numeric chunks returned
 * by crb_ipv6_split(). Each range must use the "begin1#begin2#end1#end2" format.
 *
 * When a matching range is found, $key receives the matching item key from $list.
 *
 * @param int|float $d1 Second numeric chunk of the IPv6 address returned by crb_ipv6_split().
 * @param int|float $d2 Third numeric chunk of the IPv6 address returned by crb_ipv6_split().
 * @param array<int|string, string> $list IPv6 range list in "begin1#begin2#end1#end2" format.
 * @param int|string|null $key Receives the matched range key, or null when no match is found.
 *
 * @return bool True if the IPv6 chunks are within one of the ranges, false otherwise.
 */
function crb_ipv6_is_in_range_list( $d1, $d2, &$list, &$key = null ) {
	$key = null;

	foreach ( $list as $range_key => $v6range ) {
...
/**
 * Returns the ACL tag for an IPv6 address range match, giving priority to the allowed ACL.
 *
 * The IPv6 address is represented by the second and third numeric chunks returned
 * by crb_ipv6_split(). The first chunk is filtered by the database query before
 * this function is called.
 *
 * The function scans preloaded IPv6 ACL rows and returns the allowed tag immediately
 * when matched. Blocked matches are remembered and returned only if no allowed
 * match is found.
 *
 * When a match is found, $key receives the matching row key from $v6rows so the
 * caller can retrieve the corresponding ACL row.
 *
 * @param int|float $d1 Second numeric chunk of the IPv6 address returned by crb_ipv6_split().
 * @param int|float $d2 Third numeric chunk of the IPv6 address returned by crb_ipv6_split().
 * @param array<int|string, array{v6range:string, tag:string}> $v6rows IPv6 ACL rows. Each v6range value must use the "begin1#begin2#end1#end2" format.
 * @param int|string|null $key Receives the matched row key, or null when no match is found.
 *
 * @return false|string False if the IP address is not listed, 'B' for the blocked ACL, or 'W' for the allowed ACL.
 */
function crb_ipv6_get_tag( $d1, $d2, &$v6rows, &$key = null ) {
	...
	foreach ( $v6rows as $row_key => $row ) {
		if ( $blocked && ( $row['tag'] == 'B' ) ) {
			continue;
		}
...
/**
 * Splits an IPv6 address into three numeric chunks suitable for PHP and MySQL comparison.
 *
 * A full IPv6 address is converted to a 32-character hexadecimal string and split
 * into chunks of 15, 15, and 2 hex characters. This keeps the large IPv6 value
 * comparable without requiring native 128-bit integer support.
 *
 * @param string $ip Valid IPv6 address.
 *
 * @return array{0:int,1:int,2:int} Integer IPv6 chunks.
 */
...
register_activation_hook( cerber_plugin_file(), function () {
	...
	if ( is_user_logged_in() ) {  // Not for remote plugin installation/activation
		...
		if ( ! crb_get_settings( 'no_white_my_ip' ) ) {
			...
			$whited = ' <p>' . sprintf( __( 'Your IP address %s has been added to the Allowed IP Access List', 'wp-cerber' ), cerber_get_remote_ip() );
		}
	...
	cerber_update_set( CRB_ADMIN_WIDE, $msg );
...
function cerber_upgrade_db( $force = false ) {
	...
	if ( ! $force && $db_errors ) {
		CRB_Issues::add( __FUNCTION__, 'Database errors occurred while upgrading WP Cerber database tables.', array( 'section' => 'upgrading_db', 'type' => CRB_Issues::TYPE_EVENT, 'context' => $db_errors ) );
...
register_shutdown_function( function () {
	...
	CRB_Bug_Hunter::handle_shutdown_error( error_get_last() );
...
function cerber_inspect_value( $value = '', $reset = false ) {
	...
	if ( $varbyref = cerber_is_base64_encoded( $value ) ) {
		...
		if ( ! $found && CRB_JS_Detector::cerber_detect_js_code( $value ) ) {
...
function cerber_detect_php_code( $value ) {
	...
	$clean = preg_replace( "/[\r\n\s]+/", ' ', cerber_remove_comments( (string) $value ) );

	if ( false === strpos( $clean, '<?php' ) ) {
		$clean = '<?php ' . $clean;
	}

	$tokens = @token_get_all( $clean );
...
function cerber_push_the_news() {
	...
	$announcement[] = crb_ui_element( 'p', array( 'style' => 'text-align:right; padding-right: 20px;' ),
		crb_ui_element( 'input', array(
			'type' => 'button',
			'class' => 'button button-primary crb-dismiss-trigger',
			'data-crb-context' => 'new_release_announcement',
			'value' => " \u{00A0} " . __( 'Awesome!', 'wp-cerber' ) . " \u{00A0} ",
		) )
	);

	$text = crb_ui_renderer()->render_element( crb_ui_fragment( $announcement ) );

	update_site_option( CRB_ADMIN_ANNOUNCE, $text );
}
