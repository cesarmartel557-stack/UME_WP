...
// A common suffix used for query tab ID parameter for every addon. Do not refactor this constant ever.
const CRB_ADDON_SIGN = '_addon';
// And admin page ID shared by all addons. Do not refactor this constant ever.
const CRB_ADDON_PAGE = 'cerber-addons';

CRB_Events::add_handler( 'update_settings', function ( $data ) {
...
function crb_update_add_on_list() {
	if ( ! $addons = CRB_Addons::get_all() ) {
		cerber_update_set( CRB_Addons::BOOT_ADDONS, array() );
	...
	cerber_update_set( CRB_Addons::BOOT_ADDONS, $boot );
}
...
final class CRB_Events {
	...
	static function event_handler( $event, $data ) {
		...
		if ( ! isset( self::$addon_files ) ) {
			if ( ! self::$addon_files = cerber_get_set( CRB_Addons::BOOT_ADDONS ) ) {
...
final class CRB_Addons {
	...
	// Addons to load
	public const BOOT_ADDONS = 'boot_cerber_addons';

	private static array $addons = array();
	private static array $tabs = array(); // Add-on tab IDs, they are equal add-on setting screen IDs
	 ...
	 *
	 * @return bool True on successul addon registration
	 */
	...
	/**
	 * Build and returns setting screens and sections for all add-ons
	 *
	 * @return array[]|false
	 */
	static function get_settings_config() {
	...
	/**
	 * Returns add-on tab ID for the specified settings screen ID
	 *
	 * @param string $settings_screen_id
	 *
	 * @return string
	 *
	 * @since 9.6.2.1
	 */
	static function get_addon_tab( $settings_screen_id ): string {
		return self::$tabs[ $settings_screen_id ] ?? '';
	}

	/**
	 * Returns settings screen ID for the specified add-on tab (or the add-on page)
	 *
	 * @param string $id
	 *
	 * @return string
	...
	static function load_active() {
		if ( ! $list = cerber_get_set( self::BOOT_ADDONS ) ) {
			return;
		}
	...
	static function get_admin_ui() {
		...
		$config = array(
			...
			'callback' => function ( $tab, $tab_data ) {
				CRB_Settings_Renderer::render_form( $tab, $tab_data );
			},
		);
