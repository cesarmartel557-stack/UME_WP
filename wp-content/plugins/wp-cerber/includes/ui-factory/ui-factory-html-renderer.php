...
// New since 9.6.9.8
			...
			// --- Structural nodes: no HTML tag of their own, only their children are rendered ---
			case 'fragment':
			case 'element_set':
				return $this->render_children( $element->children );

			// Handle plain text content
			case 'text':
				return crb_escape_html( $element->props['content'] ?? '' );

			// Plain-text template with numbered placeholders and typed arguments
			case 'formatted_text':
				return $this->render_formatted_text( $element->props );
	...
	protected function render_generic_tag( CRB_UI_Element $element ): string {
		...
		if ( ! empty( $element->children ) ) {
			$inner_html = $this->render_children( $element->children );
		}
	...
	/**
	 * Renders a list of child elements in their original order.
	 *
	 * Structural nodes such as 'fragment' and 'element_set' compile directly
	 * into their children. Each child renderer owns the escaping of its own
	 * output context, so this method adds no escaping of its own, no wrapper
	 * element, and no separators or whitespace between children.
	 *
	 * @param CRB_UI_Element[] $children Child elements to render.
	 *
	 * @return string Concatenated HTML of all rendered children.
	 *
	 * @since 9.8.4.1
	 */
	private function render_children( array $children ): string {
		...
		foreach ( $children as $child_element ) {
			$children_html .= $this->render_element( $child_element );
		}
	...
	/**
	 * Renders a plain-text template with numbered placeholders.
	 *
	 * The template itself is treated as plain text, not HTML. Literal template
	 * fragments and scalar arguments are escaped for HTML output. UI element
	 * arguments are rendered through the current renderer, so each dynamic value
	 * is escaped according to its own output context.
	 *
	 * Only numbered placeholders in the form %N$s are supported. The args array
	 * is a zero-based positional list, so %N$s maps to the argument at index
	 * N - 1. Missing arguments render as an empty string and extra arguments are
	 * ignored.
	 *
	 * @param array{template?: string, args?: array<int, mixed>} $props Formatted text properties.
	 *
	 * @return string Safe HTML fragment.
	 *
	 * @since 9.8.2
	 */
	private function render_formatted_text( array $props ): string {
		...
		// Render each argument once as a zero-based positional list, so placeholder
		// %N$s maps to argument index N - 1 regardless of the caller's array keys.

		$placeholder_arguments = array_values( (array) ( $props['args'] ?? [] ) );
		$rendered_arguments = [];

		foreach ( $placeholder_arguments as $argument_index => $placeholder_argument ) {
			$rendered_arguments[ $argument_index ] = $this->render_formatted_text_argument( $placeholder_argument );
		}

		return $this->render_text_with_placeholders( $template, $rendered_arguments );
	}

	/**
	 * Renders one formatted-text placeholder argument.
	 *
	 * UI elements are rendered as typed UI output. All other arguments are
	 * treated as plain text and escaped for HTML output.
	 *
	 * @param mixed $placeholder_argument Placeholder argument.
	 *
	 * @return string Safe HTML fragment for the placeholder.
	 *
	 * @since 9.8.2
	 */
	private function render_formatted_text_argument( $placeholder_argument ): string {

		if ( $placeholder_argument instanceof CRB_UI_Element ) {
			return $this->render_element( $placeholder_argument );
		}

		return crb_escape_html( (string) $placeholder_argument );
	}

	/**
	 * Interpolates a plain-text template with rendered placeholder fragments.
	 *
	 * Literal template fragments are escaped as text. Placeholder replacements
	 * are already rendered by render_formatted_text_argument().
	 *
	 * @param string $template Plain-text template with numbered placeholders.
	 * @param array<int, string> $rendered_arguments Rendered placeholder fragments.
	 *
	 * @return string Safe HTML fragment.
	 *
	 * @since 9.8.2
	 */
	private function render_text_with_placeholders( string $template, array $rendered_arguments ): string {

		if ( ! preg_match_all( '/%([1-9][0-9]*)\$s/', $template, $placeholder_matches, PREG_OFFSET_CAPTURE ) ) {
			return crb_escape_html( $template );
		}

		$result_html = '';
		$current_offset = 0;

		foreach ( $placeholder_matches[0] as $match_index => $placeholder_match ) {
			$placeholder_token = $placeholder_match[0];
			$placeholder_offset = $placeholder_match[1];

			// Escape the literal fragment preceding this placeholder.

			$result_html .= crb_escape_html( substr( $template, $current_offset, $placeholder_offset - $current_offset ) );

			// Map placeholder number N to the zero-based argument index N - 1.

			$placeholder_number = (int) $placeholder_matches[1][ $match_index ][0];
			$argument_index = $placeholder_number - 1;

			if ( array_key_exists( $argument_index, $rendered_arguments ) ) {
				$result_html .= $rendered_arguments[ $argument_index ];
			}

			$current_offset = $placeholder_offset + strlen( $placeholder_token );
		}

		// Escape the trailing literal fragment after the last placeholder.

		$result_html .= crb_escape_html( substr( $template, $current_offset ) );
