...
/**
 * Recursively converts objects to associative arrays.
 *
 * Object properties are extracted using get_object_vars(), so only properties
 * visible from the current scope are included.
 *
 * Scalar values inside arrays and objects are preserved unchanged.
 * Non-array and non-object top-level values are converted to an empty array.
 *
 * This function is intended for simple DTO-like values. It does not protect
 * against circular object references.
 *
 * @param mixed $input_value Value to normalize.
 *
 * @return array Normalized array with nested objects converted to arrays.
 */
function crb_normalize_to_array_deep( $input_value ): array {
	if ( is_object( $input_value ) ) {
		$input_value = get_object_vars( $input_value );
	}
	...
	foreach ( $input_value as $property_name => $property_value ) {
		if ( is_object( $property_value ) || is_array( $property_value ) ) {
			$input_value[ $property_name ] = crb_normalize_to_array_deep( $property_value );
		}
	}
