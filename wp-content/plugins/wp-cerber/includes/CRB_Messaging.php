...
final class CRB_Messaging {
	 ...
	 *
	 * @param string $msg_type Message type identifier.
	 ...
	 * @since 9.6.16
	 */
	public static function send( $msg_type, $msg_parts = array(), $channels = array(), $ignore = false, $args = array() ) {
		return self::instance()->send_message( $msg_type, $msg_parts, $channels, $ignore, $args );
	}
	 ...
	 * @since 9.5.5.1
	 */
	private function send_message( $msg_type, $msg_parts = array(), $channels = array(), $ignore = false, $args = array() ) {

		$msg_type = $msg_type ?: 'generic';
		...
		if ( ! $ignore
		     && ! is_admin()
		     && in_array( $msg_type, array( 'lockout', 'send_alert' ) ) ) {
		...
		if ( ! in_array( $msg_type, array( '2fa', 'new_version' ) ) ) {
		...
		switch ( $msg_type ) {
			case 'citadel_mode':
				...
				break;
			case 'lockout':
				$max = cerber_db_get_var( 'SELECT MAX(stamp) FROM ' . CERBER_LOG_TABLE . ' WHERE  activity IN (' . CRB_EV_IPL . ',' . CRB_EV_NEL . ')' );

				if ( $max ) {
					$last_date = cerber_date( $max, false );
					$last = cerber_db_get_row( 'SELECT * FROM ' . CERBER_LOG_TABLE . ' WHERE stamp = ' . $max . ' AND activity IN (' . CRB_EV_IPL . ',' . CRB_EV_NEL . ')', MYSQL_FETCH_OBJECT );
				}
				else {
					$last_date = '';
				...
				$active = crb_get_locked_out_num();

				if ( $last->ip && ( $block = crb_get_lockout( $last->ip ) ) ) {
					$reason = $block->reason;
				}
				else {
		...
		if ( $channels['email'] ) {
			$to_list = crb_build_email_recipients( $msg_type, $args );
			$to = implode( ', ', $to_list );
		}
		...
		if ( ( 1 > $mf )
		     && ! in_array( $msg_type, array( 'shutdown', 'generic', '2fa' ) )
		...
		if ( $channels[ $go ] && ! $html_mode ) {
			$body_go = ( $msg_type == 'send_alert' && crb_get_settings( 'pb_mask' ) ) ? $body_masked : $body;
		...
		$go = 'email';
		if ( $channels[ $go ] ) {
			$body_go = ( $msg_type == 'send_alert' && crb_get_settings( 'email_mask' ) ) ? $body_masked : $body;
			if ( $results[ $go ] = $this->send_email( $msg_type, $html_mode, $to_list, $subj, $body_go, $more, $footer, $ip ) ) {
				$recipients[ $go ] = $to;
				$success = true;
			}
		}
