...
final class CRB_DS {
	private static string $setting = '_crb_ds_shadowing';
	...
	static function enable_shadowing( $type ) {
		...
		switch ( $type ) {
			case 1: // Users data
				...
				CRB_Deferred_Tasks::add( '_crb_ds_background', array( 'exec_until' => 'done', 'retry_limit' => 1000 ) );
	...
	static function acc_processor( $mode, $user_id, $data = null ) {
		...
		$update_permitted = ( crb_get_settings( 'ds_4acc_acl' ) && crb_acl_is_allowed() );
	...
	static function setting_processor( &$value, $option, &$old_value ) {
		...
		if ( crb_get_settings( 'ds_4opts_acl' ) && crb_acl_is_allowed() ) {
	...
	static function role_update_permitted( &$value, &$old_value ): bool {
		...
		if ( crb_get_settings( 'ds_4roles_acl' ) && crb_acl_is_allowed() ) {
			return true;
		}
	...
	/*static function acc_update_permitted_by_ip() {
		if ( crb_get_settings( 'ds_4acc_acl' ) && crb_acl_is_allowed() ) {
			return true;
		}
