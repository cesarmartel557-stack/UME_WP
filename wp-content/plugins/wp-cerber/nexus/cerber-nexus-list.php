		...
		if ( $which == 'top' ) {
            ...
            <div class="alignleft actions">
				...
				if ( count( $groups ) > 1 ) {
					...
					$filter .= cerber_select( 'filter_group_id', $groups, crb_array_get_validated( $_GET, 'filter_group_id', '-1', '\d+' ) );
				}
				...
				if ( count( $servers ) > 1 ) {
					...
					$filter .= cerber_select( 'filter_server_id', $list, crb_array_get_validated( $_GET, 'filter_server_id', '*', '[\w\.\:]+' ) );
				}
				...
				if ( count( $countries ) > 1 ) {
					...
					$filter .= cerber_select( 'filter_country', $list, crb_array_get_validated( $_GET, 'filter_country', '*', '\w+' ) );
				}
