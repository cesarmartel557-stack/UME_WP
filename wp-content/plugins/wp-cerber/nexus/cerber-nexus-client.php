...
function nexus_prepare_response() {
	...
	switch ( $master->type ) {
		...
		case 'submit':
			...
			if ( $master->get_post_fields( CRB_SETTINGS_SCREEN_ID_FIELD ) ) { // Plugin settings form
				return nexus_process_settings_form( $master->get_post_fields() );
			}
			else {
				return crb_process_admin_request( $master->is_post );
			}
...
function nexus_process_settings_form( array $form_fields ) {
	...
	if ( ! $settings_screen_id
	     || ! CRB_Settings_Registry::get_config( array( 'settings_screen_id' => $settings_screen_id ) ) ) {
