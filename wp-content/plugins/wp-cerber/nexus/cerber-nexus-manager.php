...
const CRB_NX_MANAGED = 'managed-edit-form'; // ID of a special form for editing managed site data
	...
	static function send( $request, $client_id = null ) {
		...
		if ( ! crb_is_wp_error( $network ) ) {
			...
			if ( ! empty( $network['payload']['error'] ) ) { // A critical error on the remote website
				$m = 'An error occurred on ' . $website->site_name . ': ' . crb_escape_html( $network['payload']['error'][1] );
			...
			$causes = array(
				...
				'The IP address of this main website does not match the one set in the access settings on the managed website',
			...
			if ( $network->get_error_code() == 'nexus_http_error' ) {
				...
				$error = crb_escape_html( $network->get_error_message() );
			}
            ...
            <div style="padding: 4em;">
                ...
                <ul style="list-style: disc;">
					...
					if ( ! $kb_filter = crb_array_get( $causes_map, $network->get_error_code() ) ) {
						if ( ! $kb_filter = crb_array_get( $causes_map, self::$last_http ) ) {
							$kb_filter = null;
						}
					}
				...
				if ( $http_content = $network->get_error_data() ) {
					...
					echo '<div class="crb-monospace" style="width: 100%; max-height: 50vh; overflow: auto; background-color: #fff; border: 2px solid #aaa; padding: 1em;">' . crb_escape_html( $http_content ) . '</div>';
				}
				?>
...
add_filter( 'set-screen-option', function ( $status ) {
	...
	if ( isset( $_POST['nexus_slave_list_screen'] ) ) {
		$set             = get_site_option( '_cerber_slist_screen', array() );
		$set['url_name'] = crb_array_get_validated( $_POST, 'crb_url_in_name', 0, '\d' );
		$set['srv_ip']   = crb_array_get_validated( $_POST, 'crb_srv_ip', 0, '\d' );
...
function nexus_show_site_edit_form( $site_id ) {
	...
	// Sections config for the edit form, built at render time from the client website data
	...
	// The form is rendered and processed by the internal form engine.
	// The submission is dispatched to nexus_save_client_data_form().
	CRB_Settings_Renderer::render_form( CRB_NX_MANAGED, array(), $edit_fields );
}

/**
 * Saves the data submitted via the managed-site edit form (CRB_NX_MANAGED).
 *
 * Updates the client website data stored on the main Cerber.Hub website:
 * resolves the selected website group or creates a new one, sanitizes the
 * website owner details, updates the website row via nexus_update_client_data(),
 * and removes website groups that are no longer used.
 *
 * Called by cerber_process_settings_form() after the form nonce and the
 * management permission have been verified.
 *
 * @param array<string,string> $form_fields Submitted form fields (unslashed and trimmed)
 *
 * @return bool True if the form has been processed, false if the request context
 * is not eligible or the submitted website ID is invalid
 *
 * @since 9.8.4
 */
function nexus_save_client_data_form( array $form_fields ) {

	// Preserve the security boundary of the removed options.php saving path
	...
	$site_id = absint( $form_fields['site_id'] ?? 0 );
	...
	// Resolve the website group: an existing group ID or a new group name to create

	$group_id = 0;

	if ( ! empty( $form_fields['site_group'] ) ) {
		$groups = nexus_get_groups();

		if ( is_numeric( $form_fields['site_group'] ) ) {
			$group_id = (int) $form_fields['site_group'];
		}

		if ( ! $group_id || ! isset( $groups[ $group_id ] ) ) {
			// Add a new group
			$new_group_name = strip_tags( $form_fields['site_group'] );
	...
	// Sanitize the website owner details and update the website data

	$owner_details = array_intersect_key( $form_fields, array_flip( nexus_client_form_fields() ) );
	$owner_details = array_map( 'strip_tags', $owner_details );

	$client_data = array_replace( $form_fields, $owner_details );
	$client_data['group_id'] = $group_id;

	if ( nexus_update_client_data( $site_id, $client_data ) ) {
		cerber_admin_message( __( 'Website data has been updated', 'wp-cerber' ) );
	}
	else {
		cerber_admin_notice( __( 'Unable to update website data', 'wp-cerber' ) );
	}
...
function nexus_add_client( $token ) {
	...
	$data['site_name']        = mb_substr( crb_escape_html( htmlspecialchars_decode( $t[6] ) ), 0, 250 );
...
function nexus_update_client_data( $id, $data ) {
	...
	if ( isset( $data['site_name'] ) ) {
		$data['site_name'] = crb_escape_html( $data['site_name'] );
	}
...
function nexus_delete_client( $ids ) {
	...
	if ( $ret ) {
        ...
        if ( $num === 1 ) {
            $msg = __( 'Website has been deleted', 'wp-cerber' );
        }
        else {
            /* translators: %d is the number of deleted websites. */
            $msg = sprintf( _n( '%d website has been deleted', '%d websites have been deleted', $num, 'wp-cerber' ), $num );
        }
        ...
        if ( $plugins = crb_array_get( $nums, 'plugins' ) ) {
	        ...
	        foreach ( $plugins as $plugin => $data ) {
		        ...
		        if ( ! cerber_get_set( $key, null, false ) ) {
			        // Defense-in-depth: managed-site payloads are untrusted. Strip any tags from
			        // string fields at the trust boundary. Output escaping still happens on render,
			        // so values are stored raw (un-entity-encoded) to avoid double-encoding.
			        foreach ( array( 'Name', 'Version', 'Description' ) as $f ) {
				        if ( isset( $data[ $f ] ) && is_string( $data[ $f ] ) ) {
					        $data[ $f ] = strip_tags( $data[ $f ] );
				        }
			        }
			        $data['plugin_slug'] = strip_tags( (string) $plugin );
...
function nexus_set_context() {
	...
	if ( $client = nexus_get_client_data( $id ) ) {
		if ( crb_get_settings( 'master_swshow' ) ) {
			/* translators: %s is the name of the website the user switched to. */
...
function nexus_bg_upgrade( $ids, $plugins ) {
	...
	foreach ( $ids as $id ) {
		$ret = CRB_Deferred_Tasks::add( 'nexus_do_upgrade', array(
	...
	if ( $sites = cerber_db_get_col( 'SELECT id FROM ' . cerber_get_db_prefix() . CERBER_MS_TABLE . ' WHERE refreshed < ' . $t . ' AND last_http <= 200 LIMIT 50' ) ) {
		...
		foreach ( $sites as $id ) {
			...
			*/
			...
			//CRB_Deferred_Tasks::add( array( 'func' => 'nexus_send', 'args' => array( array( 'type' => 'hello' ), $id ) ) );
...
function nexus_add_bg_refresh( $client_id ) {
    CRB_Deferred_Tasks::add( 'nexus_send', array( 'args' => array( array( 'type' => 'hello' ), $client_id ), 'load_admin' => 1 ) );
    CRB_Deferred_Tasks::add( 'nexus_refresh_client_servers', array( 'args' => array( $client_id ) ) );
}
...
add_action( 'wp_before_admin_bar_render', function () {
	...
	if ( $current = nexus_get_context() ) {
		...
		/* translators: %s is the current website name. */
		$title = sprintf( __( 'You are here: %s', 'wp-cerber' ), $current->site_name );
	}
	else {
        ...
        jQuery(function ($) {
            ...
            $("#crb-nexus-sites .bulkactions .button").on('click', function (e) {
                if ('nexus_delete_slave' === $(this).prev('select').find(':selected').val()) {
                    if (!confirm('<?php echo crb_escape_string_for_js_in_html( __( 'Are you sure you want to delete selected websites?', 'wp-cerber' ) ); ?>')) {
                        e.preventDefault();
                    }
                }
            });
...
add_action( 'wp_ajax_cerber_master_ajax', function () {
	...
	switch ( crb_array_get( $_REQUEST, 'crb_ajax_do' ) ) {
		case 'nexus_view_updates':
			...
			// Site name is stored escaped; normalize it so we escape exactly once for the output context.
			$client    = nexus_get_client_data( $client_id );
			$site_name = ( $client && isset( $client->site_name ) ) ? html_entity_decode( $client->site_name, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8' ) : '';
			$header    = __( 'Active plugins and updates on', 'wp-cerber' ) . ' ' . $site_name;
			...
			if ( $list = nexus_get_client_plugins( $client_id ) ) {
				...
				foreach ( $list as $item ) {
					...
					if ( $update = crb_array_get( $item, 'update' ) ) {
						...
						//	dashicons-info
						$upd = '<span style="color: green;"><i class="dashicons dashicons-info"></i> ' . __( 'A newer version is available', 'wp-cerber' ) . ' (' . esc_html( crb_array_get( $update, 'new_version' ) ) . ')</span>';
					...
					$tbody .= '<tr class="' . esc_attr( $c ) . '" data-plugin-slug="' . esc_attr( crb_array_get( $data, 'plugin_slug' ) ) . '"><td><p style="font-size: 1.09em;"><b>' . esc_html( crb_array_get( $data, 'Name' ) ) . ' ' . esc_html( crb_array_get( $data, 'Version' ) ) . '</b> ' . $upd . '</p><p>' . esc_html( crb_array_get( $data, 'Description' ) ) . '</p></td></tr>';
				...
				*/
				...
				$refreshed = ( $client && isset( $client->refreshed ) ) ? cerber_auto_date( $client->refreshed ) : __( 'Unknown', 'wp-cerber' );
				$html = esc_html( __( 'No information available. Refreshed:', 'wp-cerber' ) . ' ' . $refreshed );
            }

			// Build the complete, pre-escaped popup markup server-side so the browser
			// never concatenates dynamic values into HTML. All html MUST BE inside of "crb-popup-wrap".
			$response = array(
				'html' => '<div id="crb-popup-wrap"><div id="crb-outer"><div id="crb-inner"><h3>' . esc_html( $header ) . '</h3>' . $html . '</div></div><p class="crb-popup-controls"><input type="button" value="OK" class="crb-mpopup-close button button-primary"></p></div>',
			);
			break;
	}
...
function nexus_create_db( $role ) {
	...
	$sql = array();
	if ( ! cerber_db_is_table( cerber_get_db_prefix() . CERBER_MS_TABLE ) ) {
		$sql[] = '
	...
	if ( ! cerber_db_is_table( cerber_get_db_prefix() . CERBER_MS_LIST_TABLE ) ) {
		$sql[] = '
...
function nexus_upgrade_db( $force = false ) {
	...
	if ( $force || ! cerber_db_is_column( cerber_get_db_prefix() . CERBER_MS_TABLE, 'server_id' ) ) {
	...
	if ( $force || ! cerber_db_is_column( cerber_get_db_prefix() . CERBER_MS_TABLE, 'site_key' ) ) {
		$sql[] = 'ALTER TABLE ' . cerber_get_db_prefix() . CERBER_MS_TABLE . '
