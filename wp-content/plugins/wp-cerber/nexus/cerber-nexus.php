...
function nexus_site_manager() {
	if ( nexus_is_main() ) {
		...
		$disable_link = _crb_admin_link_add( [ 'cerber_admin_do' => 'nexus_set_role', 'nexus_set_role' => 'none' ] );
		...
		echo '<p>' . __( 'To disable remote management and revoke the token, click here:', 'wp-cerber' ) . ' ' . crb_confirmation_link( $disable_link, __( 'Disable managed mode', 'wp-cerber' ), __( 'Are you sure? This permanently invalidates the token.', 'wp-cerber' ) ) . '.</p>';

		echo '</div>';

		CRB_Settings_Renderer::render_form( 'nexus-slave' );
	}
}
...
/**
 * Check if the request is coming from a main Cerber.Hub website.
 * Performs all checks and validates main website credentials.
 *
 * @return bool True if the website is configured as managed (client) AND the current request is valid and originated from an authentic main website.
 */
...
/**
 * Determine whether the current admin request is executing in a remote client context.
 *
 * Returns true when the website admin has selected a remote website on a main Cerber.Hub website.
 *
 * @return bool True if the current admin request is executing in a remote client context.
 *
 * @since 9.7.1
 */
function nexus_is_remote_context(): bool {
	return (bool) nexus_get_context();
}

/**
 * Retrieve remote client data for the current admin request when a remote Cerber.Hub website is selected.
 *
 * @return false|object False if the current admin request is not executing in a remote client context,
 *                      otherwise an object containing the remote website data.
 */
function nexus_get_context() {
	static $client_data, $client_id;
	...
	if ( ! $client_data = nexus_get_client_data( $client_id ) ) {
		$client_id = null;
		$client_data = false;
	}
