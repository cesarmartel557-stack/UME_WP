...
add_action( 'plugins_loaded', 'crb_scan_run_scheduled', 0 );

function crb_scan_run_scheduled() {
	...
	if ( isset( $_POST['scan_mode'] ) ) {
		...
		if ( cerber_is_cloud_enabled( $mode ) ) {
			...
			if ( $do ) {
				$scanner = crb_scan_run_scanner( $do, $mode );
...
function crb_scan_run_scanner( $control, $mode ) {
	global $cerber_scan_mode;

	if ( crb_get_settings( 'scan_debug' ) ) {
		register_shutdown_function( function () {
			if ( http_response_code() != 200 ) {
				crb_scan_diagnostic_log( 'Unexpected software errors detected. Check the web server error log.', true );
				if ( $err = error_get_last() ) {
					crb_scan_diagnostic_log( print_r( $err, 1 ) );
				}
			}
		} );

		Revalt::set_error_logger( function ( $code, $message, $data ) {
			if ( $data === null ) {
				$details = '';
			}
			...
			crb_scan_diagnostic_log( $message . ' [ERROR CODE: ' . $code . '] ERROR DATA: ' . $details, true );
		}, Revalt::LOG_INSTANT );
	}
	...
	switch ( $control ) {
		case 'start_scan':
			if ( cerber_init_scan( $mode ) ) {
				crb_scan_diagnostic_log( '>>>>>>>>>>>>>>> START SCANNING v. ' . CERBER_VER . ', mode: ' . $mode . ', memory: ' . @ini_get( 'memory_limit' ) );
	...
	if ( $scan = cerber_get_scan() ) {
		...
		if ( ! cerber_is_cloud_request() ) {
			...
			if ( $duration < 3600 ) {
				$ret['duration'] = gmdate( "i:s", $duration );
			}
			else {
				$hours = intdiv( $duration, 3600 );
				$minutes = intdiv( $duration % 3600, 60 );
				$seconds = $duration % 60;
				$ret['duration'] = sprintf( "%02d:%02d:%02d", $hours, $minutes, $seconds );
			}
...
function cerber_step_scanning() {
	...
	crb_scan_diagnostic_log( cerber_get_stage_title( $stage ) . ' (step: ' . $current_step . ' stage: ' . $stage . ')' );
	...
	switch ( $stage ) {
		...
		case 5:
			...
			while ( ! $aborted && ! $exceed && ! $done ) {
				...
				$chunk_result = $db->table( cerber_get_db_prefix() . CRB_SCANFILES_TABLE )
				                   ->select( 'file_name', 'scan_id', 'file_name_hash' )
				...
				$result = $chunk_result->has_errors() ? array() : $chunk_result->get_results( array() );
			...
			$parsed_result = $db->table( cerber_get_db_prefix() . CRB_SCANFILES_TABLE )
			                    ->select_count( 'scan_id' )
			                    ->where( 'scan_id', '=', $scan_id )
			                    ->where( 'file_type', '!=', 0 )
			                    ->get_value();

			$update['total']['parsed'] = $parsed_result->has_errors() ? 0 : $parsed_result->get_results( 0 );
		...
		default:
			if ( $stage != CRB_SCAN_FIN ) {
				crb_scan_diagnostic_log( 'Unknown (not defined) scan stage: ' . $stage, true );
			}
	...
	$scanned_result = $db->table( cerber_get_db_prefix() . CRB_SCANFILES_TABLE )
	                     ->select_count( 'scan_id' )
	                     ->where( 'scan_id', '=', $scan_id )
	                     ->where( 'scan_status', '>', 0 )
	                     ->get_value();

	$update['scanned']['files'] = $scanned_result->has_errors() ? 0 : $scanned_result->get_results( 0 );

	$scanned_bytes_result = $db->table( cerber_get_db_prefix() . CRB_SCANFILES_TABLE )
	                           ->select_sum( 'file_size' )
	                           ->where( 'scan_id', '=', $scan_id )
	                           ->where( 'scan_status', '>', 0 )
	                           ->get_value();

	$update['scanned']['bytes'] = $scanned_bytes_result->has_errors() ? 0 : $scanned_bytes_result->get_results( 0 );

	if ( $remain ) {
		crb_scan_diagnostic_log( 'Remain objects at the stage: ' . $remain );
	}

	if ( $stage_progress ) {
		crb_scan_diagnostic_log( 'Stage progress: ' . $stage_progress . '%' );
	}

	if ( isset( $update['total']['files'] ) ) {
		crb_scan_diagnostic_log( 'Files total: ' . $update['total']['files'] );
	}

	if ( isset( $update['total']['parsed'] ) ) {
		crb_scan_diagnostic_log( 'Parsed files: ' . $update['total']['parsed'] );
	}

	if ( $update['scanned']['files'] ) {
		crb_scan_diagnostic_log( 'Scanned files: ' . $update['scanned']['files'] );
	}
	...
	if ( isset( $update['finished'] ) ) {
		...
		if ( crb_is_wp_error( $cr ) ) {
			crb_scan_diagnostic_log( $cr );
		}
	}

	if ( isset( $update['aborted'] ) ) {
		crb_scan_diagnostic_log( '>>>>>>>>>>>>>>> SCANNING HAS BEEN ABORTED' );
	}
	elseif ( isset( $update['finished'] ) ) {
		crb_scan_diagnostic_log( '>>>>>>>>>>>>>>> SCANNING HAS FINISHED' );
	}
...
function cerber_scan_completed() {
	...
	$report = crb_scan_build_email_report( $scan );

	if ( ! $report ) {
		crb_scan_diagnostic_log( 'No issues for email reporting found.' );

		return;
	}

	if ( ! CRB_Messaging::send( 'scan', array( 'text' => $report ) ) ) {
		crb_scan_diagnostic_log( 'Unable to send email report', true );
	}
	else {
		crb_scan_diagnostic_log( 'Email report has been sent.' );
	}
...
function cerber_before_scan() {
	...
	if ( @is_dir( $dir ) &&
		...
		crb_scan_diagnostic_log( 'Cleaning up in the session directory ' . $dir );
	...
	if ( @is_dir( $dir ) &&
		...
		crb_scan_diagnostic_log( 'Cleaning up in the temp directory ' . $dir );
		cerber_empty_folder( $dir );
	}
...
function cerber_empty_folder( $dir ) {
	...
	if ( crb_is_wp_error( $r ) ) {
		...
		crb_scan_diagnostic_log( $r );
	}
	else {
		crb_scan_diagnostic_log( 'Directory has been emptied: ' . $dir );
	}
}
...
function cerber_apply_scan_policies() {
	...
	crb_scan_diagnostic_log( 'Cleaning up...' );
		...
		foreach ( $set['issues'] as $key => &$issue ) {
			...
			if ( $delete ) {
				...
				if ( crb_is_wp_error( $result ) ) {
					...
					crb_scan_diagnostic_log( 'File deleted: ' . $file_name );
	...
	if ( $update ) {
		crb_scan_diagnostic_log( 'Updating scan data...' );
	...
	foreach ( $scan['issues'] as $id => &$set ) {
		foreach ( $set['issues'] as $key => &$issue ) {
			...
			if ( crb_is_wp_error( $source_file ) ) {
				...
				crb_scan_diagnostic_log( $source_file );
			...
			if ( crb_is_wp_error( $result ) ) {
				...
				crb_scan_diagnostic_log( $result );
				$issue['data']['prced'] = CERBER_FRCF;

				continue;
			}

			if ( ! wp_is_writable( $file_name )
			     || ! @copy( $source_file, $file_name ) ) {

				crb_scan_diagnostic_log( 'Unable to recover file: ' . $file_name, true );

				if ( $err = error_get_last() ) {
					crb_scan_diagnostic_log( $err['message'], true );
				}

				$issue['data']['prced'] = CERBER_FRCF;
			}
			else {
				crb_scan_diagnostic_log( 'The file has been recovered: ' . $file_name );
...
function cerber_get_the_source( $package_type, $file_name, $data = array() ) {
	...
	crb_scan_diagnostic_log( 'Downloading source: ' . $url );
	...
	if ( ( ! $zip_file || crb_is_wp_error( $zip_file ) )
		...
		crb_scan_diagnostic_log( 'Failed. Downloading from alternative source: ' . $url );
...
function cerber_cleanup_recovery() {
	...
	if ( ! $fs->rmdir( $folder . CRB_SCAN_RCV_DIR, true ) ) {
		return new WP_Error( 'cerber-dir', 'Unable to clean up the recovery directory' . ' ' . $folder . CRB_SCAN_RCV_DIR );
	}
...
/**
 * Returns the latest known scan ID.
 *
 * If a scan is currently in progress, its ID is the latest known scan ID.
 *
 * @return int|false Scan ID, or 0 when no scan has ever been launched.
 */
function cerber_get_scan_id(): int {

	$scan_ids = warp_run_db( static function ( CRB_Database $db ): Revalt {
		return $db->table( cerber_get_db_prefix() . CERBER_SETS_TABLE )
		          ->select( 'the_id' )
		          ->where( 'the_key', '=', 'scan' )
		          ->get_column();
	} );
	...
	if ( ! $scan_ids->has_errors() ) {
		$all = $scan_ids->get_results();

		if ( $all ) {
			$scan_id = (int) max( $all ); // There is no index for the_id column, so it should be faster.
		}
	}
...
function cerber_get_scan( $scan_id = null ) {
    ...
    // If no scan ID is specified, look for the latest one
	if ( $scan_id === null ) {
	...
	$scan_data = cerber_get_set( 'scan', $scan_id );

	// Build message for UI
	$scan_data['mode_h'] = ( $scan_data['mode'] == 'full' ) ? __( 'Full Scan', 'wp-cerber' ) : __( 'Quick Scan', 'wp-cerber' );

	if ( ! empty( $scan_data[ CRB_CHUNK_COUNT ] ) ) {

		// Retrieve chunked data

		$chunks = (int) $scan_data[ CRB_CHUNK_COUNT ];
		...
		$rows_result = warp_run_db( static function ( CRB_Database $db ) use ( $keys, $scan_id ): Revalt {
			return $db->table( cerber_get_db_prefix() . CERBER_SETS_TABLE )
			          ->select( 'the_key', 'the_value' )
			          ->where( 'the_key', 'IN', $keys )
			          ->where( 'the_id', '=', $scan_id )
			          ->get_query_results();
		} );

		$rows = $rows_result->has_errors() ? array() : $rows_result->get_results( array() );

		// Map by key, then reassemble in known order. Copy-on-write keeps the chunk
		// strings shared across $rows/$map/$values, so this does not duplicate the
		// payload. Free the helpers before the heavy unpack below.

		$map = array();
		foreach ( $rows as $row ) {
			$map[ $row['the_key'] ] = $row['the_value'];
		}
		unset( $rows );
		...
		if ( $values ) {
			if ( ! empty( $scan_data['compressed'] ) && extension_loaded( 'zlib' ) ) {
	...
	if ( $length > $chunk_size ) {
		...
		if ( extension_loaded( 'zlib' ) ) {
			if ( $issues = bin2hex( gzcompress( $issues, 2 ) ) ) {
				...
				crb_scan_diagnostic_log( "Chunk is compressed {$length} {$gzlength} " . ( $length / $gzlength ) );
				$length = $gzlength;
				$data['compressed'] = 1;
			}
		}
		...
		$data[ CRB_CHUNK_COUNT ] = $index - 1;
		unset( $issues );
		crb_scan_diagnostic_log( 'Split data into ' . $data[ CRB_CHUNK_COUNT ] . ' chunks, chunk size ' . $chunk_size );
	}
...
function cerber_delete_old_scans() {
	$scans_result = warp_run_db( static function ( CRB_Database $db ): Revalt {
		return $db->table( cerber_get_db_prefix() . CERBER_SETS_TABLE )
		          ->where( 'the_key', '=', 'scan' )
		          ->order_by( 'the_id', 'DESC' )
		          ->get_query_results();
	} );

	$scans = $scans_result->has_errors() ? array() : $scans_result->get_results( array() );
...
function cerber_delete_scan( $scan_id ) {
	...
	if ( ! empty( $scan[ CRB_CHUNK_COUNT ] ) ) {
		for ( $n = 0; $n <= $scan[ CRB_CHUNK_COUNT ]; $n ++ ) {
	...
	warp_run_db( static function ( CRB_Database $db ) use ( $scan_id ): Revalt {
		return $db->table( cerber_get_db_prefix() . CRB_SCANFILES_TABLE )
		          ->where( 'scan_id', '=', $scan_id )
		          ->delete();
	} );
...
function cerber_get_num_files( $scan_id ) {
	$count_result = warp_run_db( static function ( CRB_Database $db ) use ( $scan_id ): Revalt {
		return $db->table( cerber_get_db_prefix() . CRB_SCANFILES_TABLE )
		          ->select_count( 'scan_id' )
		          ->where( 'scan_id', '=', absint( $scan_id ) )
		          ->get_value();
	} );

	return $count_result->has_errors() ? 0 : $count_result->get_results( 0 );
}
 ...
 * @since 9.6.2.6
 */
function cerber_get_issue_labels( int $id = 0 ) {
	$list = array(
		CERBER_FOK    => __( 'Verified', 'wp-cerber' ),
...
/**
 * Scanner messages, warnings, notifications
 *
...
function crb_scan_messages() {
	return array(
		...
		'finished'        => __( 'The scan has finished', 'wp-cerber' ),
		'finished_desc'   => __( 'The scan has finished. Please review the results.', 'wp-cerber' ),
	);
}
...
function cerber_log_scan_error( $msg = '' ) {
	...
	crb_scan_diagnostic_log( $msg, true );
...
function crb_check_abandoned( &$stage_progress, $num = 2 ) {
	...
	return CRB_Scan::iterate_sections_stateful( 3, function ( $section ) {
		...
		crb_scan_diagnostic_log( 'Checking if abandoned: ' . $section['name'] . ' (' . $slug . ')' );

		$status = CRB_Plugin::get_plugin_repo_status( $slug, crb_get_settings( 'scan_abon_pl_period' ) );

		crb_scan_diagnostic_log( 'Result: ' . $status['status_msg'] . ' (code ' . $status['sts_code'] . ')' );
...
function crb_check_ownership( int &$stage_progress, int $num = 2 ) {
	...
	return CRB_Scan::iterate_sections_stateful( 3, function ( $section ) {
		...
		crb_scan_diagnostic_log( 'Checking ownership changes: ' . $section['name'] . ' (' . $slug . ')' );
		...
		if ( crb_is_wp_error( $ownership ) ) {
			...
			if ( $error == 'Plugin not found.' ) { // Plugin no hosted on wp.org
				crb_scan_diagnostic_log( 'Plugin not found on wordpress.org.' );
			}
			else {
				crb_scan_diagnostic_log( $error, true );
			}
		...
		if ( count( $history ) <= 1
			...
			crb_scan_diagnostic_log( 'No changes in ownership since ' . $last_date );

			return false;
		}

		crb_scan_diagnostic_log( 'Warning: plugin owner has changed.' );
		...
		$prev_profile = $prev['contributors'][ $prev['owner'] ] ?? array();
		$new_profile  = $new['contributors'][ $new['owner'] ] ?? array();

		$prev_owner_link = crb_ui_link(
			$prev_profile['profile'] ?? '',
			$prev_profile['display_name'] ?? $prev['owner'],
			array(
				'target' => '_blank',
		...
		$new_owner_link = crb_ui_link(
			$new_profile['profile'] ?? '',
			$new_profile['display_name'] ?? $new['owner'],
			array(
				'target' => '_blank',
				'rel'    => 'noopener noreferrer',
			)
		...
		/* translators: %1$s: the date and time of the scan (e.g., "August 28, 2025, 9:23:04 PM"), %2$s: the previous owner name (e.g., "Albert"), %3$s: the new owner name (e.g., "John"). */
		$msg = crb_ui_renderer()->render_element(
			crb_ui_formatted_text(
				__( 'At %1$s the scanner detected that the ownership of the plugin changed from %2$s to %3$s.', 'wp-cerber' ),
				array(
					$last_date,
					$prev_owner_link,
					$new_owner_link,
...
function cerber_verify_plugins( &$progress ) {
	...
	while ( ! empty( $to_scan ) ) {
		...
		crb_scan_diagnostic_log( 'Verifying the plugin: ' . $plugins[ $plugin ]['Name'] . ' ' . $plugins[ $plugin ]['Version'] );
		...
		if ( ! $verified ) {
			...
			if ( $plugin_hash && ! crb_is_wp_error( $plugin_hash ) ) {
				foreach ( $plugin_hash['files'] as $file => $hash ) {
					...
					$local_file_result = $db->table( cerber_get_db_prefix() . CRB_SCANFILES_TABLE )
					                        ->where( 'scan_id', '=', $scan_id )
					                        ->where( 'file_name_hash', '=', $file_name_hash )
					                        ->get_row();

					$local_file = $local_file_result->has_errors() ? false : $local_file_result->get_results( array() );
					...
					$db->table( cerber_get_db_prefix() . CRB_SCANFILES_TABLE )
					   ->where( 'scan_id', '=', $scan_id )
					   ->where( 'file_name_hash', '=', $file_name_hash )
					   ->update( array(
						   'file_hash_repo' => $file_hash_repo,
...
function cerber_verify_plugin( $plugin_slug, $plugin_data, $local_only = false ) {
	...
	if ( $hash ) {
		...
		crb_scan_diagnostic_log( 'Using local hash...' );
		...
		if ( ! $issues ) {
			crb_scan_diagnostic_log( 'Success!' );
		}
...
function cerber_inspect_php( $file_name = '' ) {
	...
	foreach ( $pos as $k => $p ) {
		...
		$code_lines = $code_lines + array_slice( $lines, $p[0], $length, true );
	}
...
function cerber_get_strings() {
	...
	$data['explain'] = array(
		...
		__( 'The scanner recognized this file as "ownerless" or "not bundled" because it does not belong to any known part of the website and should not be here.', 'wp-cerber' ),
	...
	$data['explain_issue'] = array(
		CERBER_LDE => array(
			array(
				...
				/* translators: %s is the name of the plugin, theme, or WordPress core. */
...
function cerber_verify_files( $hash_data, $field = 'file_hash', $local_prefix = '', $type_not_in = array(), $set_type = null, $func = null ) {
	...
	foreach ( $hash_data as $file_name => $hash ) {
		...
		$update_result = $db->table( cerber_get_db_prefix() . CRB_SCANFILES_TABLE )
		                    ->where( 'scan_id', '=', $scan['id'] )
		                    ->where( 'file_name_hash', '=', $file_name_hash )
		                    ->update( array(
			                    'file_type'      => $file_type,
...
function cerber_obtain_hash( $url, $nocache = false ) {
	...
	while ( $n < 3 ) {
		...
		$pause = 3 + $n;
		crb_scan_diagnostic_log( 'Rate limiting in effect. Taking a pause for ' . $pause . ' seconds.' );
		sleep( $pause );
		$n ++;
	}
...
function crb_net_download_hash( $url, $nocache = false ) {
	...
	crb_scan_diagnostic_log( 'Launching cURL to download integrity data from: ' . $url );
	...
	if ( $result ) {
		...
		if ( 200 === $http_code ) {
			...
			$size = strlen( $payload );
			crb_scan_diagnostic_log( 'Integrity data downloaded from: ' . $url );
			crb_scan_diagnostic_log( 'SIZE: ' . $size );
		...
		elseif ( 429 === $http_code ) {
			...
			crb_scan_diagnostic_log( 'To many requests (HTTP 429).' );
		}
...
function cerber_add_file_info( $file ) {
	...
	if ( $type == CERBER_FT_CNT && is_link( $file['file_name'] ) ) {
	    ...
	    if ( is_dir( $file_name ) ) {
			...
			$count_result = $db->table( cerber_get_db_prefix() . CRB_SCANFILES_TABLE )
			                   ->select_count( 'scan_id' )
			                   ->where( 'scan_id', '=', $file['scan_id'] )
			                   ->where( 'file_name', '=', $file_name )
			                   ->get_value();

			$delete_it = $count_result->has_errors() ? 0 : $count_result->get_results( 0 );
		}

		if ( $delete_it ) {
			$delete_result = $db->table( cerber_get_db_prefix() . CRB_SCANFILES_TABLE )
			                    ->where( 'scan_id', '=', $file['scan_id'] )
			                    ->where( 'file_name_hash', '=', $file['file_name_hash'] )
			                    ->delete();

			return ! $delete_result->has_errors();
		}
	...
	// Assemble the fields to update; file_name is included only when a symlink was resolved to a real file name
	...
	if ( $update_file_name !== '' ) {
		$update_fields['file_name'] = $update_file_name;
	}
...
/**
 * Update miscelanous file fields
 *
 * @param string $file_name_hash Hash of the filename
 * @param array $fields Database row's columns to update
...
function cerber_scan_update_fields( $file_name_hash, $fields, $scan_id = null ) {
	...
	return warp_run_db( static function ( CRB_Database $db ) use ( $scan_id, $file_name_hash, $fields ): Revalt {
		return $db->table( cerber_get_db_prefix() . CRB_SCANFILES_TABLE )
		          ->where( 'scan_id', '=', $scan_id )
		          ->where( 'file_name_hash', '=', $file_name_hash )
		          ->update( $fields );
...
/**
 * Retrieves the ID of the previous completed scan that matches the current scan mode.
 *
 * @param int $scan_id The ID of the current scan. Defaults to 0 if not provided.
 *
 * @return int The ID of the previous scan if found, or 0 if no match is found.
 */
function cerber_get_prev_scan_id( $scan_id = 0 ) {
	...
	$scans_result = warp_run_db( static function ( CRB_Database $db ) use ( $scan_id ): Revalt {
		return $db->table( cerber_get_db_prefix() . CERBER_SETS_TABLE )
		          ->select( '*' )
		          ->where( 'the_key', '=', 'scan' )
		          ->where( 'the_id', '<', $scan_id )
	...
	if ( $scans_result->has_errors() ) {
		return 0;
	}

	$scans = $scans_result->get_results();
...
function cerber_cmp_scans( $prev_id, $scan_id ) {
	...
	$p_files_result = $db->table( cerber_get_db_prefix() . CRB_SCANFILES_TABLE )
	                     ->select( 'file_name', 'file_name_hash', 'file_hash', 'file_md5', 'file_size' )
	                     ->where( 'scan_id', '=', $prev_id )
	                     ->get_query_results();

	$p_files = $p_files_result->has_errors() ? array() : $p_files_result->get_results( array() );

	$n_files_result = $db->table( cerber_get_db_prefix() . CRB_SCANFILES_TABLE )
	                     ->select( 'file_name', 'file_name_hash', 'file_hash', 'file_md5', 'file_size' )
	                     ->where( 'scan_id', '=', $scan_id )
	                     ->get_query_results();

	$n_files = $n_files_result->has_errors() ? array() : $n_files_result->get_results( array() );
	...
	foreach ( $update as $key => $status ) {
		$db->table( cerber_get_db_prefix() . CRB_SCANFILES_TABLE )
		   ->where( 'scan_id', '=', $scan_id )
		   ->where( 'file_name_hash', '=', $key )
		   ->update( array( 'file_status' => $status ) );
	}
...
function _crb_save_file_names( $list ) {
	...
	foreach ( $list as $filename ) {
		...
		if ( ! $count_result->has_errors() && $count_result->get_results( 0 ) ) {
			continue;
		}
		...
		if ( isset( $ignore[ $file_name_hash ] ) ) {
			$status = 1;
			crb_scan_diagnostic_log( 'The file is in the ignore list: ' . $filename );
		}
	...
	$insert_result = $db->table( cerber_get_db_prefix() . CRB_SCANFILES_TABLE )
...
function cerber_need_for_hash( $zip_file = '', $delete = true, $expires = 0 ) {
	...
	foreach ( $files as $zip_file ) {
		...
		crb_scan_diagnostic_log( 'Processing ZIP: ' . cerber_mb_basename( $zip_file ) );

		$result = crb_hash_maker( $zip_file, $tmp_folder1, false, $expires );

		if ( crb_is_wp_error( $result ) ) {

			crb_scan_diagnostic_log( 'Processing ZIP: ' . $result->get_error_message() );

		    // It's possible that there is a nested ZIP archive

			if ( $nested_zip_list = glob( $tmp_folder1 . '*.zip' ) ) {

			    crb_scan_diagnostic_log( 'Processing ZIP: trying to find the reference code in the nested zip archive' );
			...
			crb_scan_diagnostic_log( 'Processing ZIP: ' . cerber_mb_basename( $zip_file ) . ' - OK!' );
		}
	...
	crb_scan_diagnostic_log( 'Processing ZIP: Completed' );
...
function crb_hash_maker( $zip_file, $zip_folder, $delete = true, $expires = 0 ) {
	...
	if ( $err ) {
		/* translators: %s is the uploaded file name. */
...
function cerber_get_the_folder( $return_as_is = false ) {
	...
	CRB_Issues::delete_item( __FUNCTION__ );
...
function cerber_get_my_folder() {
	...
	if ( defined( 'CERBER_FOLDER_PATH' )  ) { // @since 9.5.1
		...
		crb_check_dir( $path, $error, 'Directory defined by the constant CERBER_FOLDER_PATH does not exist or is outside the allowed paths: ' . crb_escape_html( $user_defined ) );
	...
	if ( ! $path ) {
		$none_msg = 'Required WordPress uploads directory does not exist: ' . $path;
...
function cerber_quarantine_file( $file_name, $scan_id, $move = true ) {
	...
	if ( ! crb_move_copy( $file_name, $new_name, $move ) ) {
		...
		if ( $move ) {
			$msg = 'Unable to move file to the quarantine: ' . $file_name . '. Check permissions (owner) of this directory: ' . $dir;
		}
		else {
			$msg = 'Unable to copy file to the quarantine: ' . $file_name . '. Check permissions (owner) of this directory: ' . $dir;
		}
...
/**
 * Return the ID number of the node if the request is originated from the Cerber Cloud, false otherwise
 *
 * @return bool|integer
 */
function cerber_is_cloud_request() {
	...
	if ( ! cerber_is_http_post()
	     || ! $cloud_key = $_POST['cerber-cloud-key'] ?? '' ) {
		$ret = false;
 ...
 * @param array $scan
 *
 * @return bool|string User reprt in HTML format, false if there is nothing to report
 */
function crb_scan_build_email_report( $scan ) {
	global $cerber_scan_mode;

	// What issues to include in the report (combined list configured in the scanner settings)
	$include = crb_get_settings( 'scan_reinc' );
	// Extract severity IDs to include, severities are within the range 0-3
	$severities = array_intersect_key( array( 0, 1, 2, 3 ), $include );
	// Types of issues to include - other than severities, 4 and above
	$types = array_keys( $include );

	if ( ! $last_filtered = cerber_filter_issues( $scan, $types, $severities ) ) {
		return false;
	}
	...
	if ( $prev_scan_id = cerber_get_prev_scan_id( $scan['id'] ) ) {
		$prev_scan = cerber_get_scan( $prev_scan_id );
	}
	else {
		$prev_scan = null;
    }
	...
	if ( $re > 1 ) {
		if ( $prev_scan ) {
			$prev_filtered = cerber_filter_issues( $prev_scan, $types, $severities );
		}
	}
	...
	foreach ( $for_report as $section_id => $section ) {
		...
		foreach ( $section['issues'] as $issue ) {
			...
			if ( $isize ) {
				$size_diff = '';
				if ( in_array( CERBER_NEW, $issue['ii'] ) && $prev_scan_id ) {
					$file_name_hash = sha1( $issue['data']['name'] );

					$psize_result = warp_run_db( static function ( CRB_Database $db ) use ( $prev_scan_id, $file_name_hash ): Revalt {
						return $db->table( cerber_get_db_prefix() . CRB_SCANFILES_TABLE )
						          ->where( 'scan_id', '=', $prev_scan_id )
						          ->where( 'file_name_hash', '=', $file_name_hash )
						          ->get_value( 'file_size' );
					} );

					$previous_size = $psize_result->has_errors() ? false : $psize_result->get_results( '' );

					if ( is_numeric( $previous_size ) ) {
						$diff = $issue['data']['bytes'] - $previous_size;
						if ( absint( $diff ) > 0 ) {
	...
	$report_html .= '<table style="border-collapse: collapse; ' . $css_table . '"><tr>' . implode( '</tr><tr>', $rows ) . '</tr></table>';

	// Errors

	if ( crb_get_settings( 'scan_ierrors' ) && $ers = cerber_get_scan_errors()) {
		$report_html .= '<table style="' . $css_table . '"><tr><td style="' . $css_td . ' font-size:80%;" ><p style="font-weight: 600; margin:0;">Some errors occurred during the scan</p><ol style="padding-left: 1em;"><li>' . implode( '</li><li>', $ers ) . '</li></ol></td></tr></table>';
	}
	...
	$header_html  = '<div style="' . $css_table . '"><div style="margin:0 auto; text-align: center;"><p style="font-size: 130%; padding-top: 0.5em;">' . $site_name . '</p><p style="">' . $mode . '</p><div style="padding-bottom: 1em;">' . $summary_html . '</div></div></div>';

	return '<div style="width:100%; padding: 1em; margin:0; text-align: center; background-color: #f9f9f9;">' . $header_html . $report_html . '</div>';
}
...
function cerber_get_vulnerabilities( $plugin_slug, $plugin ) {
	...
	if ( ! $vu_list
		...
		crb_scan_diagnostic_log( 'Getting vulnerability data from the cloud.' );
 ...
 */
function crb_scan_diagnostic_log( $msg, $as_error = false ) {
	if ( ! crb_get_settings( 'scan_debug' ) ) {
		return;
	}
...
function crb_file_sanitize( &$issues ) {
	foreach ( $issues as &$section ) {
		...
		foreach ( $section['issues'] as &$issue ) {
			if ( ! empty( $issue[1] ) ) {
				$issue[1] = crb_escape_html( $issue[1] );
			}
			if ( ! empty( $issue['data']['name'] ) ) {
				$issue['data']['name'] = crb_escape_html( $issue['data']['name'] );
			}
		}
	}
}
...
final class CRB_Scan_Grinder {
	...
	static function is_wp_media_file( $file_name ) {
		...
		if ( ! isset( $cache[ $file_name ] ) ) {
			$attachment_result = warp_run_db( static function ( CRB_Database $db ) use ( $wpdb, $file_name ): Revalt {
				return $db->table( $wpdb->postmeta )
				          ->alias( 'pm' )
				          ->join( $wpdb->posts, 'pm.post_id', 'p.ID', 'INNER', 'p' )
				          ->where( 'pm.meta_key', '=', '_wp_attached_file' )
				          ->where( 'pm.meta_value', '=', $file_name )
				          ->get_row();
			} );

			$attachment_row = $attachment_result->has_errors() ? array() : $attachment_result->get_results( array() );
			$cache[ $file_name ] = ( $attachment_row ) ? true : false;
		}
	...
	static function check_web_access( $file_name ) {
		...
		crb_scan_diagnostic_log( 'Checking web access to ' . $file_name . ' via ' . $file_url );
		...
		while ( $attempts ) {
			...
			if ( $status ) {
				crb_scan_diagnostic_log( $status );
			}
	...
	private static function iterator( $file_processor, $file_type = null, $scan_status = array( 0, CERBER_UOP ) ) {
		...
		$scan_status = array_map( 'absint', $scan_status );
		$step = cerber_scan_get_step();
		$table = cerber_get_db_prefix() . CRB_SCANFILES_TABLE;
		...
		// Chunk SELECT. WHERE conditions are duplicated below for the one-time COUNT.
		$chunk_builder = $db->table( $table )
		                    ->where( 'scan_id', '=', self::$scan_id )
		                    ->where( 'scan_status', 'IN', $scan_status )
		                    ->where( 'scan_step', '!=', $step );

		if ( $file_type ) {
			$chunk_builder->where( 'file_type', '=', absint( $file_type ) );
		}

		$chunk_result = $chunk_builder->limit( CRB_SQL_CHUNK )->get_query_results();

		if ( $chunk_result->has_errors() ) {
			return 0;
		}

		$files = $chunk_result->get_results_list();

		if ( ! $files ) {
			return 0;
		}

		// Total is computed once per scan, then cached in CRB_SCAN_TEMP below.
		if ( $first_chunk ) {
			$count_builder = $db->table( $table )
			                    ->where( 'scan_id', '=', self::$scan_id )
			                    ->where( 'scan_status', 'IN', $scan_status )
			                    ->where( 'scan_step', '!=', $step );

			if ( $file_type ) {
				$count_builder->where( 'file_type', '=', absint( $file_type ) );
			}

			$count_result = $count_builder->select_count()->get_value();

			if ( $count_result->has_errors() ) {
				return 0;
			}

			$total_files = (int) $count_result->get_results( 0 );
		}

		$num = count( $files );

		crb_scan_diagnostic_log( 'Files to process: ' . $num );
		...
		if ( $issues ) {
			...
			crb_scan_diagnostic_log( 'Issues found: ' . $inum );
		}
