...
//do_action('bookingpress_' . $requested_module . '_dynamic_view_load');
    ...
    add_action('bookingpress_add_dynamic_buttons_for_view_appointments', [$this,'add_appointment_stories_view_button'], 10);
...
*/
        ...
        public function add_appointment_stories_view_button(){
            global $BookingPressPro, $bookingpress_pro_staff_members;
            ...
            <el-link :href="'javascript:window.open(`?page=bookingpress_historias&appointment='+scope.row.appointment_id+'&paciente_id='+scope.row.bookingpress_customer_id+'&paciente='+(scope.row.custom_fields_values.find(el=> el.label='text_C6kufq').value )+'`, `_top`)'"
            style='' 
        ...
        function bookingpress_customer_vue_data_fields(){
            ...
            if( !in_array($request_module, [$historiasClinicas_module_name, 'appointments']) ){
                #echo "<h1>RETORRRNAAA $historiasClinicas_module_name</h1>";
            return;
            }
        ...
        function bookingpress_customer_dynamic_on_load_methods_func()
        {
            ...
            if( !empty($_GET['appointment'])){
                ...
                echo " this.selectPatient( {'customer_id': $current_pid, 'dni': $current_dni } ); ";
        ...
        function bookingpress_customer_dynamic_vue_methods_func()
        {
            ...
            async loadCustomersOrig() {
                ...
                var postData = { action:'bookingpress_get_customers', perpage:5/*this.perPage*/, currentpage:this.currentPage, search_data: bookingpress_search_data,_wpnonce:'<?php echo esc_html(wp_create_nonce('bpa_wp_nonce')); ?>' };
            ...
            bookingpress_upload_customer_avatar_func(response, file, fileList){
                ...
                if(response != ''){
                    ...
                    {'url' : response.upload_url, 'name': response.upload_file_name, 
                    'size': parseInt(file.size / 1024), 'type': file.type }
                    );
                }
            },
            ...
            bphc_checkUploadedFile(file){
                ...
                if( true || (file.type != 'image/jpeg' && file.type != 'image/png' && file.type != 'image/webp') ){
                    ...
                    var bpa_image_size = parseInt(file.size / 100000000000000);
                    if(bpa_image_size > 1){
                        vm2.$notify({
                            title: '<?php esc_html_e('Error', 'bookingpress-appointment-booking'); ?>',
            ...
            searchByDni( appointment_dni = 0 ){
                const vm = this;
                const dni = ( (appointment_dni?appointment_dni : vm.dni_query) || '').toString().trim().replace(/\D+/g,'');
                if(!dni){
...
response.data = {
    ...
    "edit_data": {
        ...
        "avatar_url": "https://foatconcept.com.ar/turnos/wp-content/uploads/bookingpress/1757597662_1757597657_descarga_avatar.png",
        "customer_metadata": {
    ...
    tempItem.size = "SinDatos";
            ...
            bp_hc_applyStaffmemberData( historyRecord = {} ){
                ...
                console.log('historyr staffid '+historyRecord.bookingpress_staff_member_id)
                 ...
                 /*'bookingpress_appointment_id' : 'jjeee',*/
            ...
            bphc_edit_HystoryRecord( edit_id = '0', historyObj = null, selected_appointment='' ){
                this.toggleBusy();
                ...
                this.newRecord.time = moment().format("HH");    //La Hora se guarda en formato 24hs
                    ...
                    */
                    ...
                    if(!historyObj) historyObj = this.bp_hc_HistoriesSummary.find((history) => history.id == bphc_edit_id );
...
/** SALGOOO AL ENTRAR NOMAS */
        ...
        function bookingpress_get_patient_histories(){
            ...
            $sql = "SELECT * FROM `$tableConsultas` c WHERE bookingpress_customer_id='$customer_id' ORDER BY created_at DESC limit 10";
            ...
            if( is_wp_error($result) ){
                ...
                foreach($result as $k=> $data){
                    ...
                    if( $raw != null ){
                        ...
                        $data->raw = $raw;
                        $histories[] = array_merge( (array) $data, $raw['consultation_data'],  $raw);
                    }
                    ...
                    (object) [
                            ...
                            "consultation_data" => [
                                    "general" =>[
                            			...
                            			"tratamiento" => "ninguno - no tiene nada",
                            			"notas" => "Este viene a joder 2 x 3 Y alguna otra vez (actualizado)",
                                        "archivos" => [ array("name" =>"examen.png","size"=>"500","type"=>"jpg","url"=>'https://cdn.prod.website-files.com/5dd6c916acc1cc42476f2149/60a9af48c867f6ee401a48e1_Nimbo%20nueva%20funcionalidad%20template%202020-01.png') ]
                                    ],
                                	"vitales" =>[
                                        "altura" =>"",
                                        "peso" =>"",
