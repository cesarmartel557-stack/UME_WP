<div class="el-card" style="padding: 0;background: rgb(255, 255, 255);border: 0px solid rgb(204, 208, 212);width: 100%;box-sizing: border-box;background: #f2f3f5;">
<?php if( true || isset($_GET['ingresotest']) ){ ?>
<div v-if="typeof impre !='undefined' && impre" style="width: 100%;min-height: 200px;border: 0px solid greenyellow;">
<ImpresionIngreso :ingreso_formdata="ingreso_formdata"></ImpresionIngreso>
...
<template>
    <div class="scoped_styles2" style="display: none;" v-pre></div>
        ...
        <div class="ume-wrap-2">
        
        <div class="el-card box-card" style="background: white;padding-bottom: 20px;border: 0;">
        
            <el-row type="flex" class="bpa-mlc-head-wrap" style="padding: 40px 30px;">
		      <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="bpa-mlc-left-heading">
        			<h1 class="bpa-page-heading">
                        <span><!--Formulario de -->Ingreso </span>
                        <span > 
                            <span v-if="Number(ingreso_formdata.id)"  style="color: var(--bpa-dt-black-300);"> Nro. # {{ingreso_formdata.id}} </span>
                            <span v-else  style="color: var(--bpa-dt-black-300);"> Nuevo </span>
                        </span>
                    </h1>
        		</el-col>
        		<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
        			<div class="bpa-hw-right-btn-group">
                        ...
                        */ ?>
                        ...
                        <el-button type="primary" class="bpa-btn bpa-btn--secondary" icon="el-icon-print" @click="">
                          <span> imprimir </span>
                        </el-button>
            ...
            <?php /** 
            <div v-if="!config.formato_two" shadow="never" style=" margin-bottom: 20px; display: none;" class="is_print">
               <el-row :gutter="24" type="flex">
                <el-col :span="7">
                    Fecha: {{ ingreso_formdata.fecha_ingreso? moment(ingreso_formdata.fecha_ingreso).format("YYYY/MM/DD") : 'Sin asignar'}} 
                </el-col>
                <el-col :span="7">
                    Hora: {{ ingreso_formdata.fecha_ingreso? moment(ingreso_formdata.fecha_ingreso).format("HH:mm:ss") : 'Sin asignar'}}                    
                </el-col>
                <el-col :span="10">
           ...
           */ ?>
           ...
           <div v-if="!config.formato_two" shadow="never" style="margin-inline: 30px; margin-bottom: 20px;">
               <el-row :gutter="24" type="flex">
                <el-col :span="12">                    
                    <el-form-item label="Hora y Fecha"  prop="fecha_ingreso">
						<el-date-picker format="yyyy/MM/dd HH:mm:ss" value-format="yyyy-MM-ddTHH:mm:ss" placeholder="Ingrese fecha y Hora aaaa/mm/dd hh:mm:ss" v-model="ingreso_formdata.fecha_ingreso" class="bpa-form-control bpa-form-control--date-picker" prefix-icon="" type="datetime"  ></el-date-picker>
					</el-form-item>
                </el-col>
                <el-col :span="12">
                    <el-form-item label="Médico" prop="selected_staffmember">
                        <el-select ref="sel_selected_staff" class="bpa-form-control" v-model="ingreso_formdata.selected_staffmember" @change="ingreso_onChangeStaff" filterable collapse-tags 
          ...
          <el-collapse class="int-form-collapse"  v-model="formCollapseActiveNames" @change="handleChangeFormCollapse" :accordion="false">
              ...
              <el-collapse-item title="1. Paciente" name="1">
                ...
                <el-card v-if="formCollapseActiveNames.includes('1')" class="bpa-dialog--customer-modal bpa--is-page-non-scrollable-mob" shadow="never" >
                			...
                			<el-row type="flex" :gutter="24">
                                ...
                                <el-col :xs="24" :sm="24" :md="24" :lg="12" :xl="12">											
        							<el-form-item ref="sel_selected_customer_item" prop="selected_customer">
        								...
        								<el-select-customer ref="sel_selected_customer" class="bpa-form-control" name="selected_customer" v-model="ingreso_formdata.selected_customer"  @change="bookingpress_select_customer($event)" filterable placeholder="<?php esc_html_e( 'Start typing to fetch Customer', 'bookingpress-appointment-booking' ); ?>" remote reserve-keyword :remote-method="bookingpress_get_customer_list" :loading="bookingpress_loading"  popper-class="bpa-el-select--is-with-modal" >												
        									<el-option-customer value="add_new" label="<?php esc_html_e( 'Add New', 'bookingpress-appointment-booking' ); ?>"  v-if="config.bookingpress_edit_customers == 1">
        										<i class="el-icon-plus" ></i>
        										<span><?php esc_html_e( 'Add New', 'bookingpress-appointment-booking' ); ?></span>
        									</el-option-customer><!-- v-if="ingreso_customers_list && customer_data" -->
        									<el-option-customer  v-for="(customer_data, indx) in ingreso_customers_list" :key="customer_data.value+'-'+indx"  :label="customer_data.text" :value="customer_data.value" >
                				...
                				<el-col :xs="24" :sm="24" :md="24" :lg="12" :xl="12">
                					<div class="bpa-tf-btn-group">
                                        ...
                                        <el-button class="bpa-btn bpa-btn--primary " :class="is_display_save_loader == '1' ? 'bpa-btn--is-loader' : ''" @click="saveCustomerDetails" :disabled="is_disabled" >
                                            ...
                                            <span class="bpa-btn__label" v-else><?php esc_html_e('Agregar Paciente', 'bookingpress-appointment-booking'); ?></span>
                    ...
                    <div class="asignar-paciente-body">
                        ...
                        <el-row :gutter="32" type="flex" class="bpa-dialog-footer" style="text-align: right; padding-inline: 30px; justify-content: right;">
                            <el-col :xs="24" :sm="24" :md="24" :lg="12" :xl="12">
                            </el-col>
                            <el-col :xs="24" :sm="24" :md="24" :lg="12" :xl="12">
                                <el-button v-if="Number(customer.update_id)" class="bpa-btn bpa-btn--secundary " @click="asignarCustomer('continuar', 2)">
...
<div :class="{ inhabilitado : !Number(ingreso_formdata.selected_customer) }" :style="{opacity:(!Number(ingreso_formdata.selected_customer)? '0.8':'')}" >
              ...
              <el-collapse-item title="2. MOTIVO CONSULTA INGRESO" name="2">
                ...
                <template>
                    ...
                    <el-row class="continuar-row" type="flex" align="right" style="margin-top: 30px; justify-content: right;">
                        <el-button type="default" @click="ingresoFormNextSection(3)">Continuar</el-button>
                    </el-row>
              ...
              <el-collapse-item title="3. ANTECEDENTES DE ENFERMEDAD ACTUAL" name="3">
                ...
                <template>
                    ...
                    <el-row class="continuar-row" type="flex" align="right" style="margin-top: 30px; justify-content: right;">
                        <el-button type="default" @click="ingresoFormNextSection(4)">Continuar</el-button>
                    </el-row>
              ...
              <el-collapse-item title="4. ANTECEDENTES PERSONALES" name="4">
                ...
                <template>
                    ...
                    <el-row class="continuar-row" type="flex" align="right" style="margin-top: 30px; justify-content: right;">
                        <el-button type="default" @click="ingresoFormNextSection(5)">Continuar</el-button>
                    </el-row>
              ...
              <el-collapse-item title="5. ANTECEDENTES HEREDOFAMILIARES" name="5">
                ...
                <template>
                    ...
                    <el-row class="continuar-row" type="flex" align="right" style="margin-top: 30px; justify-content: right;">
                        <el-button type="default" @click="ingresoFormNextSection(6)">Continuar</el-button>
                    </el-row>
              ...
              <el-collapse-item title="6. Examen Físico" name="6">
                ...
                */ ?>
                      ...
                      <div class="subsection">
                          <el-form-item label="Osteomioarticular" prop="examen_fisico.osteomioarticular">
                              <el-input v-model="ingreso_formdata.examen_fisico.osteomioarticular" placeholder="..."></el-input>
                          </el-form-item>
                      </div>
                      ...
                      <el-form-item style="margin-top: 30px;">
                        <el-button type="default" @click="ingresoFormNextSection(7)">Continuar</el-button>
                      </el-form-item>
                      */ ?>
            ...
            <el-collapse-item title="7. Diagnóstico de Ingreso" name="7">
                ...
                <el-row class="continuar-row" type="flex" align="right" style="margin-top: 30px; justify-content: right;">
                    <el-button type="default" @click="ingresoFormNextSection(8)">Continuar</el-button>
                </el-row>
              ...
              <el-collapse-item title="8. Terapéutica" name="8">
                ...
                <el-card>
                    ...
                    <el-row type="flex" :gutter="24" style="margin-bottom: 10px;">
                        <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">											
                ...
                <el-row class="continuar-row" type="flex" align="right" style="margin-top: 30px; justify-content: right;display: none;">
                    <el-button type="default" @click="ingresoFormNextSection(8)">Continuar</el-button>
                </el-row>
...
*/ ?>
          ...
          <div class="form-footer" style="border-radius:4px; border:1px solid var(--el-border-color-light);">
            ...
            -->
            */ ?>
            <el-button @click="obtenerConfiguracion" type="button" class="bpa-btn bpa-btn--secundary">Recuperar llenado previo</el-button>
            <el-button @click="()=>{ localStorage.setItem('last_inter_ing_formData', JSON.stringify(this.ingreso_formdata) ); this.$notify({type:'success', message:'Guardado Local'}); this.saveInternacionForm(); }" type="button" class="bpa-btn bpa-btn--primary">Guardar Ingreso →</el-button>
