...
//INTERNACION METODOS - CUSTOMER
            ...
            async bookingpress_select_customer(bookingpress_selected_customer){
				...
				if( bookingpress_selected_customer == "add_new" || !Number(bookingpress_selected_customer) ){
				    vm.ingreso_formdata.selected_customer = bookingpress_selected_customer;
					vm.open_add_customer_modal( bookingpress_selected_customer );
				} else {
				    console.log('start await customer', Date.now(), {...vm.customer} );
                    await vm.editCustomerDetails( bookingpress_selected_customer );
                    ...
                    if(valid){
                        ...
                        .then(function(response){
                            ...
                            if (response.data.variant == 'success') {
                                ...
                                vm2.fixCustomerData();
                                vm2.loadCustomers()
                                vm2.ingreso_customers_list = vm2.ingreso_customers_list = [ {'text': String(vm2.customer.firstname+' '+vm2.customer.lastname), 'value': vm2.customer.update_id} ];
                            }
