...
<div id="bphc_component_main_container" class="bphc-main-cointainer">
    <template id="BPHC_Historias_Template">
        <div class="bk_Expansion_component">
        <strong>Historias clinicas {{ turno }}</strong>
        {{ datita }}
            <div v-html="maxHTML" style="min-height: 80px;padding: 10px;">
               zzzz
            </div>
            <button @click="loadNewHTML">Cargar nuevo HTML</button>       
    ...
    var histComponent = Vue.component('bphc-historias', {
        ...
        data(){
            return {
                datita: 'jiji',
                maxHTML : '<h3>jooJOOjoooOH</h3>'
            }
        },
        methods : {
            loadNewHTML(){
                this.maxHTML = '<div class="new"><span>HTML Nuevo cargao '+(Date.now())+'<\/span><button onclick="setTimeout(()=>{$hacelo()},10)">haceelooo<\/button><style>.new{background:green;}<\/style><script >alert(`holanda`);hacelo = function (){console.log(`nuevo html`)}< /script><\/div>';
                z_scri = document.createElement('script');
                z_scri.id = 'chichichi';
                z_scri.innerHTML = 'console.log(`JOOO joo JOOO`)';
                //document.body.appendChild( z_scri );
                
                var zx = (new DOMParser).parseFromString('<script id="ddss">alert(`holanda`);var $hacelo = function (){console.log(`nuevo html`)}<\/script>', "application/xml");
                console.log(zx.documentElement);
                document.body.appendChild( zx.documentElement.document ); 
                //document.querySelector('#newHTML').innerHTML = '<div class="new"><span>HTML Nuevo cargao '+(Date.now())+'<\/span><button onclick="setTimeout(()=>{hacelo()},10)">haceelooo<\/button><style>.new{background:green;}<\/style><script >console.log("holanda");hacelo = function (){console.log("nuevo html")}<\/script><\/div>';
            }
        }
    });
