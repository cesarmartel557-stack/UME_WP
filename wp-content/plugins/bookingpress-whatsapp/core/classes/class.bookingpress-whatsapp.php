...
if (!class_exists('bookingpress_whatsapp') && class_exists( 'BookingPress_Core')) {
    class bookingpress_whatsapp Extends BookingPress_Core {
        ...
        function bookingpress_send_whatsapp_notification($bookingpress_appointment_data) {
            ...
            $send_whatsapp_notification = !empty($bookingpress_appointment_form_fields['send_whatsapp_notification']) ? $bookingpress_appointment_form_fields['send_whatsapp_notification'] : '';
$send_whatsapp_notification = (bool) $send_whatsapp_notification;
$send_whatsapp_notification = $send_whatsapp_notification? 'true' : '';

//var_dump( $send_whatsapp_notification );
        ...
        function bookingpress_send_whatsapp_notification_after_appointment_booked($inserted_booking_id, $entry_id, $payment_gateway_data) {
            ...
            #print_r($bookingpress_appointment_data);
        ...
        function bookingpress_after_change_appointment_status_func($appointment_id, $appointment_new_status){
            ...
            if(!empty($bookingpress_appointment_data) && !empty($bookingpress_appointment_data['bookingpress_customer_id'])) {
                ...
                $is_send_whatsapp_notification = $this->bookingpress_send_whatsapp_notification($bookingpress_appointment_data);
#$is_send_whatsapp_notification = (bool) $is_send_whatsapp_notification;
#var_dump( (bool) $is_send_whatsapp_notification == 'true' );
                if(!empty($is_send_whatsapp_notification) && $is_send_whatsapp_notification == 'true') {        
...
/**                    
print_r( "---$wp_template_name---- $to_number -- $whatsapp_url ---\r\n " );          
        ...
        function bookingpress_refresh_whatsapp_template_list_callback(){
            ...
            if( 'Twilio' == $selected_wp_gateway && 'freeform' != $twilio_msg_type ){
                ...
                $args = array(
                    ...
                    'method' => 'GET',
                    'headers' => array(
                        'Authorization' => 'Bearer ' . $whatsapp_permanent_token//'EAAP8Ce13XsEBPbRGP91b3QrcnN589RK3OuPBparZC9B5k8DtkZAqZC3IEP3EvGEeVhsDUJEk1zy6TCRGGbhuVN6TpuZApyLaTwdofb15wWJFxUphZCZAyMArEemR3LgFQuyx1D64q5VmILPNZAR1SvZCe7PhRKcZCZBZCqMLgJFoEoh4Th8q8Cj2rBibA3WSLWKCQZDZD'//$whatsapp_permanent_token
                    ),
                    'body' =>array(
                        'fields' => 'name,id,status,components,language',
                        'limit' => '100',
                ...
                $whatsapp_response = wp_remote_request(
                    $whatsapp_url,
                    $args
                );
                /**
                print_r("token: " . $whatsapp_permanent_token ."\r\n" );
                
                print_r( ini_get('max_input_vars') . " - "  );
                
                print_r( ini_get('post_max_size') . " - "  );
                
                print_r( ini_get('memory_limit') . " - "  );
                
                print_r( $whatsapp_response );
                */
...
add_filter('bookingpress_modified_get_settings', function($return_setting_data='',$setting_type='',$setting_name_key=''){
        
        
        #echo "<br><br>$setting_type $setting_name_key -->> ";
        #print_r($return_setting_data);
        if( $setting_type == 'notification_setting' ){
            
            if($setting_name_key == 'whatsapp_business_access_token'){
                $return_setting_data = 'EAAP8Ce13XsEBPbRGP91b3QrcnN589RK3OuPBparZC9B5k8DtkZAqZC3IEP3EvGEeVhsDUJEk1zy6TCRGGbhuVN6TpuZApyLaTwdofb15wWJFxUphZCZAyMArEemR3LgFQuyx1D64q5VmILPNZAR1SvZCe7PhRKcZCZBZCqMLgJFoEoh4Th8q8Cj2rBibA3WSLWKCQZDZD';
            }
            if($setting_name_key == 'whatsapp_phone_number_id'){
                $return_setting_data = '359982217187458';
            }
            if($setting_name_key == 'whatsapp_business_account_id'){
                $return_setting_data = '351471971373035';
            }
            if($setting_name_key == 'bookingpress_selected_whatsapp_gateway'){
                $return_setting_data = 'Whatsapp Business';
            }
            if($setting_name_key == 'bookingpress_whatsapp_selected_phone_number_field'){
                $return_setting_data = 'phone_dVPPiV';
            }
...
include_once __DIR__ .'/bookingpress_mod.php';
...
if(!empty($_GET['test'])){
    ...
    if(!empty($_GET['testsettings'])){
    
        add_action('wp_footer',function(){
            
            global $BookingPress;
            echo " TEST Whatsapp Vars ---- "; #$BookingPress->bookingpress_get_settings() 
            ...
            print_r( [ $selected_wp_gateway, $bookingpress_whatsapp_selected_phone_number_field , $whatsapp_permanent_token  ] );
