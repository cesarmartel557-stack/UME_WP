...
#https://www.turnos.sanatoriolomalinda.com.ar/wp-admin/admin.php?page=ctmdcd-manage-prescriptions#
?>
				...
				<el-container class="bpa-table-container">
					...
					<div class="bpa-tc__wrapper" v-if="current_screen_size == 'desktop'">
						<el-table ref="multipleTable" class="bpa-manage-appointment-items" :data="items" @selection-change="handleSelectionChange" fit="false" @row-click="bookingpress_full_row_clickable" @expand-change="bookingpress_row_expand">
							...
							<el-table-column prop="booking_id" min-width="60" class-name="booking_id" label="<?php esc_html_e( 'ID', 'bookingpress-appointment-booking' ); ?>">
								<template slot-scope="scope">
                            ...
                            <?php if( !defined('BK_EXPANSION_DATE_COLUMN') ){ ?>
                            ...
                            <el-table-column prop="customer_first_name" label="<?php esc_html_e( 'Customer', 'bookingpress-appointment-booking' ); ?>" sortable sort-by="customer_first_name+' '+customer_last_name">
                                <template slot-scope="scope">
									<span>{{ scope.row.customer_first_name }} {{ scope.row.customer_last_name }}</span>
								</template>
                            </el-table-column>
									...
									<div class="bpa-table-status-dropdown-wrapper" :class="(scope.row.change_status_loader == 1) ? '__bpa-is-loader-active' : ''" v-if="bookingpress_manage_appointment == 1">
										...
										<el-select class="bpa-form-control" :class="((scope.row.appointment_status == '2') ? 'bpa-appointment-status--warning' : '') || (scope.row.appointment_status == '3' ? 'bpa-appointment-status--cancelled' : '') || (scope.row.appointment_status == '1' ? 'bpa-appointment-status--approved' : '') || (scope.row.appointment_status == '4' ? 'bpa-appointment-status--rejected' : '') || (scope.row.appointment_status == '5' ? 'bpa-appointment-status--no-show' : '') || (scope.row.appointment_status == '6' ? 'bpa-appointment-status--completed' : '') || (scope.row.appointment_status == '7' ? 'bpa-appointment-status--approved turno_en_espera' : '')" v-model="scope.row.appointment_status" placeholder="<?php esc_html_e( 'Select Status', 'bookingpress-appointment-booking' ); ?>" @change="bookingpress_change_status(scope.row.appointment_id, $event)" popper-class="bpa-appointment-status-dropdown-popper" >
											<el-option-group label="<?php esc_html_e( 'Change status', 'bookingpress-appointment-booking' ); ?>">
                                                ...
                                                <el-option v-if="(bookingpress_staff_customize_view == 1)" class="<?php esc_html_e( 'EN ATENCIÓN', 'bookingpress-appointment-booking' ); ?>"  key="9" label="<?php esc_html_e( 'EN ATENCIÓN', 'bookingpress-appointment-booking' ); ?>" value="9" :disabled="(scope.row.appointment_status != '7')"></el-option>
                                                <el-option v-if="(bookingpress_staff_customize_view == 1)" class="<?php esc_html_e( 'Completed', 'bookingpress-appointment-booking' ); ?>"  key="6" label="<?php esc_html_e( 'Completed', 'bookingpress-appointment-booking' ); ?>" value="6" :disabled="!( (scope.row.appointment_status == '7') || (scope.row.appointment_status == '9') )"></el-option>
                                                
                                                                                                
                                                <el-option v-if="(bookingpress_staff_customize_view == 1 && item.value != '6' && item.value != '9') || ( (bookingpress_staff_customize_view != 1 && item.value != '6') && (bookingpress_staff_customize_view != 1 && item.value != '9') )" :class="item.text" v-for="item in appointment_status" :key="item.value" :label="item.text" :value="item.value" :disabled="(bookingpress_staff_customize_view == 1)"></el-option>
                                                
                                                
                                                <el-option v-if="!(bookingpress_staff_customize_view == 1)" class="<?php esc_html_e( 'EN ATENCIÓN', 'bookingpress-appointment-booking' ); ?>"  key="9" label="<?php esc_html_e( 'EN ATENCIÓN', 'bookingpress-appointment-booking' ); ?>" value="9" disabled></el-option>
                                                <el-option v-if="!(bookingpress_staff_customize_view == 1)" class="<?php esc_html_e( 'Completed', 'bookingpress-appointment-booking' ); ?>"  key="6" label="<?php esc_html_e( 'Completed', 'bookingpress-appointment-booking' ); ?>" value="6" disabled></el-option>
                                                
											</el-option-group>
										</el-select>
									</div>
									<el-tag class="bpa-front-pill " :class="((scope.row.appointment_status == '2') ? '--warning' : '') || (scope.row.appointment_status == '3' ? '--info' : '') || (scope.row.appointment_status == '1' ? '--approved' : '') || (scope.row.appointment_status == '4' ? '--rejected' : '') || (scope.row.appointment_status == '5' ? '--no-show' : '') || (scope.row.appointment_status == '6' ? '--completed' : '') || (scope.row.appointment_status == '7' ? '--approved --en_espera' : '')" v-else>{{ scope.row.appointment_status_label }}</el-tag>
								</div>
								</template>
							</el-table-column>
                    ...
                    <!--
                        ...
                        -->
							<el-table-column prop="created_date" label="<?php esc_html_e( 'Creado', 'bookingpress-appointment-booking' );#esc_html_e( 'Created Date', 'bookingpress-appointment-booking' ); ?>" sortable sort-by='bookingpress_appointment_created_date'>
								<template slot-scope="scope">
									<label>{{ scope.row.created_date }}</label>	
                                            						
									<div class="bpa-table-actions-wrap" v-if="<?php if(isset($_GET['test'])) echo 'true || '; ?>( bookingpress_manage_appointment == 1 && (!scope.row.is_past_appointment || (bookingpress_staff_customize_view && ( Date.now() < (new Date(scope.row.sort_appointment_date_time * 1000).getTime()+(2*24*60*60*1000)) )  ) ) )|| bookingpress_delete_appointment == 1" > 
							...
							<el-table-column prop="appointment_status" min-width="90" label="<?php esc_html_e( 'Status', 'bookingpress-appointment-booking' ); ?>">
								<template slot-scope="scope">						
									<div class="bpa-table-status-dropdown-wrapper" :class="(scope.row.change_status_loader == 1) ? '__bpa-is-loader-active' : ''" v-if="bookingpress_manage_appointment == 1">
										...
										<el-select class="bpa-form-control" :class="((scope.row.appointment_status == '2') ? 'bpa-appointment-status--warning' : '') || (scope.row.appointment_status == '3' ? 'bpa-appointment-status--cancelled' : '') || (scope.row.appointment_status == '1' ? 'bpa-appointment-status--approved' : '') || (scope.row.appointment_status == '4' ? 'bpa-appointment-status--rejected' : '') || (scope.row.appointment_status == '5' ? 'bpa-appointment-status--no-show' : '') || (scope.row.appointment_status == '6' ? 'bpa-appointment-status--completed' : '') || (scope.row.appointment_status == '7' ? 'bpa-appointment-status--approved status-turno_en_espera' : '')" v-model="scope.row.appointment_status" placeholder="<?php esc_html_e( 'Select Status', 'bookingpress-appointment-booking' ); ?>" @change="bookingpress_change_status(scope.row.appointment_id, $event)" popper-class="bpa-appointment-status-dropdown-popper" >
											<el-option-group label="<?php esc_html_e( 'Change status', 'bookingpress-appointment-booking' ); ?>">
												<el-option v-if="(bookingpress_staff_customize_view == 1) || (bookingpress_staff_customize_view != 1 && item.value != '6') " :class="item.text" v-for="item in appointment_status" :key="item.value" :label="item.text" :value="item.value"></el-option>
											</el-option-group>
										</el-select>
									</div>
									<el-tag class="bpa-front-pill " :class="((scope.row.appointment_status == '2') ? '--warning' : '') || (scope.row.appointment_status == '3' ? '--info' : '') || (scope.row.appointment_status == '1' ? '--approved' : '') || (scope.row.appointment_status == '4' ? '--rejected' : '') || (scope.row.appointment_status == '5' ? '--no-show' : '') || (scope.row.appointment_status == '6' ? '--completed' : '') || (scope.row.appointment_status == '7' ? '--approved status-turno_en_espera' : '') " v-else>{{ scope.row.appointment_status_label }}</el-tag>
								</div>
...
<el-dialog custom-class="bpa-dialog bpa-dialog--fullscreen bpa--is-page-non-scrollable-mob" modal-append-to-body=false :visible.sync="open_appointment_modal" :before-close="closeAppointmentModal" fullscreen=true :close-on-press-escape="close_modal_on_esc">
	...
	<div class="bpa-dialog-body">
			...
			<el-row>
				<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
					...
					<div class="bpa-default-card bpa-db-card">
                        ...
                        <span class="info_int" style="margin-top: -20px;"> info medico
                        <div v-show="(configuracion_medico.desc_int!=null && configuracion_medico.desc_int!='')" class="desc_int">
                            <span v-html="configuracion_medico.desc_int"></span>
                        </div>
                        </span>
							...
							<template>								
								<div class="bpa-form-body-row">
									...
									<el-row :gutter="32">
										...
										<el-col :xs="24" :sm="24" :md="24" :lg="8" :xl="8">
											...
											<el-select class="bpa-form-control" v-model="appointment_formdata.appointment_status">
                                                ...
                                                <el-option v-if="(bookingpress_staff_customize_view == 1) || (bookingpress_staff_customize_view != 1 && status_data.value != '6') " :class="status_data.text" v-for="status_data in appointment_status" :label="status_data.text" :value="status_data.value">
									...
									<el-row :gutter="32">
                                        ...
                                        <?php do_action('mod_max_add_fields'); ?>
		...
		<div class="bpa-form-row personalizados" v-if="bookingpress_form_fields.length > 0" style="display: none;">
			<el-row>
