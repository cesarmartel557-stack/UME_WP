...
.el-select.bpa-form-control[class*='bpa-appointment-status'] {
    max-width: 100%;
}
...
window.addEventListener("load", ()=>{
    ...
    maxApp.open_add_customer_modal.call(function(){console.log("opeennnnnn booking_mod.php line <?php echo __LINE__ ?>");});
...
class bookingpress_mod_appointment_PRO Extends bookingpress_pro_appointment_bookings
{
    ...
    ###LINE CODE Comentado 2024-06-26 
        ...
        add_shortcode('on_fail_booking_template', array($this, 'on_failed_confirm_booking_change_thankyouPage'));
        
        //Hook for modify shortcode content of thank you page
				add_action('wp_ajax_bookingpress_render_thankyou_content', array($this, 'bookingpress_render_thankyou_content_func'), -1);
				add_action('wp_ajax_nopriv_bookingpress_render_thankyou_content', array($this, 'bookingpress_render_thankyou_content_func'), -1);
    ...
    public function failed_booking_content( $entry_id = 0 ){
        global $wpdb, $bookingPress_Expansion;
        $content = '';
        
        $wpdb->insert("{$wpdb->prefix}_bookingpress_other_debug_logs", array(
            'bookingpress_other_log_ref_id' => 'bookingpress_other_debug_log_entry',
            'bookingpress_other_log_type'   => 'appointment_confirm_failed_logs',
            'bookingpress_other_log_event'  => 'Booking data process failed',
            'bookingpress_other_log_event_from' => 'bookingpress_bookingform',
            'bookingpress_other_log_raw_data'   => $entry_id
               
        ) );
        
        $failed_thankyou_file = $bookingPress_Expansion->get_template_file( 'render_failed_thankyou_content.html' );
        if( file_exists($failed_thankyou_file) ){
            $content = file_get_contents( $failed_thankyou_file );
        }
        return $content;
    }
    
    public function on_failed_confirm_booking_change_thankyouPage( $atts, $content, $tag ){
        global $wpdb, $BookingPress, $tbl_bookingpress_appointment_bookings, $tbl_bookingpress_entries;
        $content = '';
        $entry_id = $generated_bookingpress_appointment_booking_id = $appointment_id = 0;       
        $bookingpress_appointment_details = [];
        ...
        if(!empty($bookingpress_uniq_id)){
            if(!empty($_COOKIE['bookingpress_cart_id'])) {
                ...
                if(!empty($appointment_id)){
                    $bookingpress_appointment_details = $wpdb->get_row($wpdb->prepare("SELECT bookingpress_appointment_booking_id FROM {$tbl_bookingpress_appointment_bookings} WHERE bookingpress_order_id = %d", $appointment_id), ARRAY_A); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Reason: $bookingpress_appointment_details is table name defined globally. False Positive alarm
                    if(!empty($bookingpress_appointment_details)){
                        $generated_bookingpress_appointment_booking_id = $bookingpress_appointment_details['bookingpress_appointment_booking_id'];
                    }
                }                    
            } else {
        ...
        if(!empty($entry_id)){
            ...
            $bookingpress_appointment_details = $wpdb->get_row($wpdb->prepare("SELECT bookingpress_appointment_booking_id FROM {$tbl_bookingpress_appointment_bookings} WHERE bookingpress_entry_id = %d", $entry_id), ARRAY_A); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Reason: $tbl_bookingpress_appointment_bookings is table name defined globally. False Positive alarm
            
            $generated_bookingpress_appointment_booking_id = !empty($bookingpress_appointment_details['bookingpress_appointment_booking_id']) ? $bookingpress_appointment_details['bookingpress_appointment_booking_id'] : '';
        }
        
        
        if( !absint($generated_bookingpress_appointment_booking_id) ){
            // add failed content
            $content = '';
            $failed_content = '';
            $failed_content = $this->failed_booking_content( $entry_id );
            ob_start();
            ?>
            <style>.bpa-front-thankyou-module-container:not(.bk-epansion-booking-failed){display: none;}</style>
		...
		function bookingpress_render_thankyou_content_func(){
		  global $bookingpress_pro_appointment_bookings;
          remove_action( 'wp_ajax_bookingpress_book_appointment_booking',         [ $bookingpress_pro_appointment_bookings, 'bookingpress_render_thankyou_content_func'] );
          remove_action( 'wp_ajax_nopriv_bookingpress_book_appointment_booking',  [ $bookingpress_pro_appointment_bookings, 'bookingpress_render_thankyou_content_func'] );
            ...
            $is_entry_id_not_appointment = absint($appointment_id);
            $generated_bookingpress_appointment_booking_id = $wpdb->get_var($wpdb->prepare("SELECT bookingpress_appointment_booking_id FROM {$tbl_bookingpress_appointment_bookings} WHERE bookingpress_entry_id = %d ", $is_entry_id_not_appointment));
            
            if( !absint($generated_bookingpress_appointment_booking_id) ){
                $bookingpress_thankyou_redirect_content = $this->failed_booking_content( $is_entry_id_not_appointment );
                if( empty($bookingpress_thankyou_redirect_content) ){
                    ...
                    $wpdb->insert("{$wpdb->prefix}_bookingpress_other_debug_logs", array(
                        ...
                        'bookingpress_other_log_event'  => 'Booking data process failed',
                        'bookingpress_other_log_event_from' => 'bookingpress_bookingform',
                        'bookingpress_other_log_raw_data'   => json_encode( array('entry'=> $is_entry_id_not_appointment) )
                           
                    ) );
                    
                    $failed_thankyou_file = $bookingPress_Expansion->get_template_file( 'render_failed_thankyou_content.html' );
                    if( file_exists($failed_thankyou_file) ){
                        $bookingpress_thankyou_redirect_content = file_get_contents( $failed_thankyou_file );
                    }
                }
            ...
            #$bookingpress_thankyou_redirect_content = "<h1>MAL</h1>";
...
#var_dump('apppp DATA   ', $bookingpress_appointment_data );
...
#global $wp_filter;
#print_r( $wp_filter['bookingpress_validate_submitted_booking_form'] );
...
#print_r( [$bookingpress_return_data, debug_backtrace()] ); exit;
...
#print_r( [$payment_gateway, $response, $bookingpress_return_data] ); exit;
                ...
                */
                ...
                if ($payment_gateway == 'on-site' && $bookingpress_service_price > 0 ) {
                    ...
                    if( $payment_gateway == 'mercagopago' ){
                        ...
                        //TEST MERCADOPAGO MERCADO PAGO MULTIPLE MERCADOPAGOS PARA DIFERENTES MEDICOS STAFFMEMBERS
...
if ( ! class_exists( 'bookingpress_pro_payment_gateways_mod' ) && class_exists( 'bookingpress_pro_payment_gateways' ) ) {
	...
	class bookingpress_pro_payment_gateways_mod Extends bookingpress_pro_payment_gateways {
	   public function __construct(){} //remove_filter( 'bookingpress_validate_submitted_booking_form', array( $this, 'bookingpress_validate_submitted_booking_form_func' ) );
...
#var_dump("CONFIRMANDO BOOKING",$bookingpress_confirm_booking_received_data);
#exit;
...
if( ! class_exists('booking_mod_dni_helper') ){
    class booking_mod_dni_helper {
        ...
        public function __construct() {
            //TEST PRUEBA 10050 99900010050
            
            add_filter( 'bookingpress_modify_entry_data_before_insert', [$this, 'mod_bookingUser_before_entry_insert'], 1);
            #apply_filters( 'bookingpress_after_modify_validate_submit_form_data', $return_data );
            add_filter( 'bookingpress_after_insert_entry_data_from_frontend', [$this, 'after_entry'], 100,2 );
        }
...
#var_dump( $bookingpress_appointment_data );
        ...
        /** OMITIMOS EMAIL Y PHONE DEL REWRITE 
            * - REWRITE SE HIZO PARA TOMAR DATOS DE LA BASE DE DATOS Y TRATAR DE UTILIZAR ESOS
                    * - PARA QUE SE PRESENTE ESTA PARTE DE INFORMACION DEL CLIENTE CON LOS MISMOS DATOS.
                        * */
                        ...
                        $reason = 'Datos de identificacion del paciente no coinciden. ' . $this->error;
                        $msg =  'Error! ' . $this->error . ' Si el problema persiste contactenos via Whatsapp: +5493644640269';
                    endif;
                                        
                }
            }else{
                $this->customer_rewrite['bookingpress_customer_id'] = 0;
                $this->customer_rewrite['bookingpress_username'] = trim($dni_value);
            }
             ...
             * FUE REALIZADO PARA AGREGAR CONTENIDO DURANTE LA RESPUESTA Y MENSAJE DE ERROR
             * EN EL FORMULARIO DE REGISTRO DE CITA DEL FRONT
             */
            
    /** FORZAR REASON A VACIO PARA QUE NO PRESENTE MENSAJE DE ERROR **** */
    $reason = '';//Los paientes pueden Volver a sacar turno con el DNI ingresando emails o telefonos diferente a los previos.
...
#var_dump( $this->customer_rewrite, $posted_data, $bookingpress_entry_details );
        ...
        function after_entry($entid, $bookingpress_appointment_data){
            //TEST PRUEBA 10050 99900010050
#var_dump( $bookingpress_appointment_data ); exit;
        }
...
if( class_exists( 'bookingpress_pro_payment_gateways_mod' ) ){
    global $bookingpress_pro_payment_gateways;
    $bookingpress_pro_payment_gateways = new bookingpress_pro_payment_gateways_mod();
}

if( class_exists( 'bookingpress_mod_appointment_PRO' ) ) $booking_mod_pro = new bookingpress_mod_appointment_PRO();
    
if( class_exists( 'bookingpress_appointment_bookings_original_mod' ) ) $booking_mod_original = new bookingpress_appointment_bookings_original_mod();
