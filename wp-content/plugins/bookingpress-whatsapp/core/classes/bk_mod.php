...
global $dni_key, $obra_social_field_key, $plan_de_obra_field_key;
$dni_key = 'text_C6kufq';
$obra_social_field_key = 'obra_soc_seguros';//'text_oO9f1B';
$plan_de_obra_field_key = 'text_o9q4Cr';

class BookingMOD_appointment{
    public $price = 0;
    
    function change_app_data( $mod_appointment_data ){
        global $obra_social_field_key, $plan_de_obra_field_key;
        $plan_de_obra_field_key = !empty($plan_de_obra_field_key)? $plan_de_obra_field_key : 'text_o9q4Cr';
        ...
        if(  is_array($mod_appointment_data['form_fields']['is_particular']) ){
                ...
                if( $is_particular ){
                    ...
                    if( empty($mod_appointment_data['form_fields'][$plan_de_obra_field_key]) ) $mod_appointment_data['form_fields'][$plan_de_obra_field_key] = "_";
...
add_action('bookingpress_after_update_appointment', 'save_obs_after_appointment',12,1 );
        ...
        function save_obs_after_appointment( $appointment_id=0 ){
			global $wpdb, $tbl_bookingpress_appointment_meta, $tbl_bookingpress_appointment_bookings, $obra_social_field_key;
...
function get_bookings_day( $medico = 0, $date = "2024-01-01", $hora="", $solo_config=false, $service_id = 0, $request_data = [] ){
...
/** ####################### AJAX GET BOOK DAYS ################################### */
        ...
        function ajax_get_bookings_day(){
            ...
            ob_start();
                $config_medico = get_bookings_day( $medico, $date, $hora, false, $service_id, $appoint_data );
            $obgc = ob_get_clean();
...
add_action('wp_ajax_get_bookings_day','ajax_get_bookings_day', 10);
add_action('wp_ajax_nopriv_get_bookings_day','ajax_get_bookings_day', 10);
...
add_action('wp_ajax_get_customer_by_dni','ajax_get_customer_by_dni');        
        ...
        function get_dni_customers( $dni_value, $dni_key='text_C6kufq', $db_results = false, $comparacion='like', $tipo_doc=''){
            ...
            //updates 2026
            $first_where = " WHERE m.`bookingpress_customersmeta_key`= '{$dni_key}'  AND c.`customer_dni` {$comparacion} '{$dni_value}{$porciento}' AND c.`customer_dni`!='' AND c.`customer_dni` IS NOT NULL ";
            $first_select = "c.bookingpress_customer_id value, customer_dni dni, bookingpress_user_firstname firstname, bookingpress_user_lastname lastname";
            //Agregado customer_dni as metavalue para asegurar compatibilidad con codigo anterior
            if( $db_results ) $first_select = "*, customer_dni as bookingpress_customersmeta_value ";
            $first_query = "SELECT {$first_select} FROM {$tbl_bookingpress_customers_meta} m join {$tbl_bookingpress_customers} c on m.bookingpress_customer_id = c.bookingpress_customer_id  {$first_where} ";
            $res = @$wpdb->get_results($first_query, ARRAY_A);
            ...
            if( !empty($res) && $tipo_doc ){
                $temp_res = [];
                
                foreach( (array) $res as $resultado ){
                    if( $resultado['tipo_doc'] == 'tipo_doc' ){
                        $temp_res[] = $resultado;
                    }
                }
                $res = !empty($temp_res)? $temp_res : $res;
            }
            ...
            foreach($res as $i=>$row ){
                $txt ="";
                //Nuevo orden de texto asignado
                //$txt = ( !empty( $row['dni'])? "Documento: ".$row['dni']." \t\n":"").( isset( $row['firstname'])? "".$row['firstname']." ":"").( isset( $row['lastname'])? "".$row['lastname']."":"");
                $txt = ( isset( $row['firstname'])? "".$row['firstname']." ":"").( isset( $row['lastname'])? "".$row['lastname']."":"");
                $txt = (!empty($txt)? $txt." \t\n":"") . ( !empty( $row['dni'])? "Documento : ".$row['dni']:"");
                $res[$i]["text"] = $txt;
            }
...
##### OBRAS Y CONFIGURACION DE MEDICOS ########
        ...
        function get_all_obras(){
            ...
            if( function_exists("get_option") ){
                ...
                if( !empty($all_obras_opt) ){
                    ...
                    return apply_filters( 'booking_Expansion_get_all_obras_filter', $all_obras_opt );
                }
            }
            
            ob_start();
            include( __DIR__ . '/all_obras.php');
            $all_obras_opt = $nuevas_obras_sociales;
            ob_get_clean();
        ...
        function get_all_seguros(){
            ...
            return apply_filters( 'booking_Expansion_get_all_seguros_filter', $all_seguros_opt );
        }
        ...
        function get_admin_config_medico($medico_id = 0){
...
*/
        ...
        /** fin merge all seguros - seguros medico */
                ...
                $config_medico = apply_filters('booking_Expansion_get_admin_config_medico', $config_medico, $medico_id);
                return $config_medico;
        }
        ...
        //fclose($f);
            ...
            $nuevas_obras = [];
                ...
                //2026 UPDATES
                do_action( 'booking_Expansion_on_save_config_medico', compact("obras", "seguros", "appoint_data") );
            ...
            if( $medico_id ){
                ...
                //echo json_encode
                wp_send_json( array("success"=> " Ajustes guardados ".($up_1?" [descripcion actualizada] ":"").($up_2?" [configuracion actualizada] ":"").($up_3?" [mp actualizado] ":"").($up_4?" [obras actualizadas] ":"").($up_5?" [seguros actualizados] ":"")." " ) );
                //ob_start();
                //$config_medico = get_admin_config_medico($medico_id);//get_bookings_day( $medico_id, "2024-01-01", "", true );
                //echo ob_get_clean(); 
                ...
                //echo json_encode
                wp_send_json( array("error"=>"Hubo un problema, No se recibio ID de medico.") );
            }
...
add_action( 'admin_print_scripts', function(){
    ...
    */
    ...
    <style>
    body.__bpa-is-staff-customize-view-active:not(.bookingpress_page_bookingpress_appointments) .bpa-table-actions {
    ...
    .__bpa-is-staff-customize-view-active .bpa-view-appointment-card .bpa-hw-right-btn-group {
    display: none !important;
    }
    body.__bpa-is-staff-customize-view-active .bookingpress_page_inner_wrapper {
        opacity: 0;
    }
    body.__bpa-is-staff-customize-view-active el-dialog {
        opacity: 0;
    }
    
    body:not(.bookingpress_page_bookingpress_appointments) .bpa-manage-appointment-items .bpa-table-status-dropdown-wrapper:before {
        content: ' ';
    ...
    body:not(.bookingpress_page_bookingpress_appointments) .bpa-manage-appointment-items .bpa-table-actions-wrap {
        display: none
    }
    span.caret-wrapper {
    ...
    <script>
    jQuery(document).ready(function(bg){
        if(bg('body.__bpa-is-staff-customize-view-active .bookingpress_page_inner_wrapper')){
            bg('body.__bpa-is-staff-customize-view-active .bookingpress_page_inner_wrapper').css('opacity','1')
        }
        if( bg('body.__bpa-is-staff-customize-view-active el-dialog') ){
            bg('body.__bpa-is-staff-customize-view-active el-dialog').css('opacity','1')
        }
    });
    </script>
...
/** AGREGADO DE ESTADOS DE TURNOS CUSTOM STATUS A LAS OPCIONES GLOBALES */
add_filter('bookingpress_add_global_option_data', function($global_data ){
    ...
    $global_data['appointment_status'][] = array(
      ...
      */
        ...
        /*
        global $bookingpress_staffmember_id;
        ...
        */
        ...
        //if( !empty($status_complete) && $bookingpress_staffmember_id ) $new[] = $status_complete;
        if( !empty($status_complete) ) $new[] = $status_complete;
        
        $new[] = array( "value"  => '9', "text"  => "EN ATENCIÓN");
...
add_filter('bookingpress_modify_appointment_data_fields', function( $bookingpress_appointment_vue_data_fields ){
    $appointment_status = $bookingpress_appointment_vue_data_fields['appointment_status'];
    
    $keys_value = array_column($bookingpress_appointment_vue_data_fields['appointment_status'], 'value');
    $keys_value = array_flip($keys_value);
    if( isset( $keys_value[6] ) ){
        $complete_indx=$keys_value[6];
        ...
        //if( !$bookingpress_staffmember_id ) unset( $bookingpress_appointment_vue_data_fields['appointment_status'][$complete_indx] );
    }
...
//Updates 2025
add_filter('bookingpress_modify_dashboard_view_file_path', function($bookingpress_load_file_name){
    if( strpos( $bookingpress_load_file_name, 'manage_dashboard.php') && is_admin() ){
        $bookingpress_load_file_name = __DIR__ . '/mod_manage_dashboard.php';
    }
...
include_once __DIR__ . '/mod_customers.php';
        ...
        //Remplaza bookingpress_get_search_customer_list_func en bookingpress_calendar
        ...
        //updates 2026 //2026 updates
        //Remplaza bookingpress_get_customer_list_func en bookingpress_calendar
        add_action('wp_ajax_bookingpress_get_customer_list', 'bk_mod_get_customer_list_func', -1);
        /**
         * Ajax request for get customer list
         *
         * @return void
         */
        function bk_mod_get_customer_list_func() {

			global $wpdb, $BookingPress, $BookingPressPro, $bookingpress_customers;
			$response                       = array();
            
            $bpa_check_authorization = $bookingpress_customers->bpa_check_authentication_helper( 'retrieve_customers', true, 'bpa_wp_nonce' );
                ...
                if( !$is_particular )
                {
                    ...
                    else
                    {
                        ...
                        if($medico_id && !empty($date) && !empty($start_time) ){
                            ...
                            $appoint_data = is_array($post_data['appointment_data'])? $post_data['appointment_data'] : [];
                            $service_id = absint(!empty($appoint_data['service_id'])? $appoint_data['service_id'] : ( !empty($appoint_data['selected_service'])? $appoint_data['selected_service'] : 0) );
                            //print_r($post_data);
                            ###$medico_booking_day = $Booking_Expansion_ObrasParticular_Staff_Settings->extend_get_bookings_day(); get_bookings_day( $medico_id, $date, $start_time, false, $service_id, $appoint_data );
                            
                            $medico_booking_day = [];
                            if( $service_id ):
                                $medico_booking_day = apply_filters('validate_config_medico_get_bookings_day', $medico_id, $date, $start_time, false, $service_id, $appoint_data );
                            endif;
                            //echo "  /* ++ ";
                            //print_r($medico_booking_day['obras_sociales']);
                            //echo "  */ "; 
                            if( empty($medico_booking_day) ){
                                //echo "  /* ****empty */  ";
                                $medico_booking_day = get_bookings_day($medico_id, $date, $start_time);
                            }
...
#var_dump(!empty($medico_booking_day['solo_particular']),strtolower($obra_soc_seguros), $is_valid,  $medico_booking_day ); exit;
                            //2025 updates
                            //Por mas que no habilite las obras agregamos verificacion previa de solo_particular habilitado Y asignamos mensaje de error especifico
                            if( !empty($medico_booking_day['solo_particular']) && "particular" != strtolower($obra_soc_seguros)){
                                $is_valid = 0;
                                $msg = sprintf( esc_html__("En esta ocacion el medico acepta solo particular. La obra social o seguro `%s` no esta disponible", "bookingpress-appointment-booking"), $obra_soc_seguros);
                            }
                            
                            //2025 updates SI AUN ES VALIDO TRAS VERIFICAR SI ES SOLO PARTICULAR
                            if( $is_valid ){
...
#var_dump($is_valid,  $post_data ); exit;
                if($retorno){
...
add_action('wp_footer', function(){
    ...
    if( isset($_GET['testcustomer']) ){
        print_r( get_dni_customers( '654', $dni_key='text_C6kufq', $db_results = true, $comparacion='like') );
    }
    ...
    <link href="https://servicioswww.anses.gob.ar/ooss2/App_Themes/estilos/AnsesNuevo.css" rel="stylesheet" type="text/css">
    <?php 
    /**
    if( file_exists(__DIR__ . '/Snoopy.class.php') ){
        include_once __DIR__ . '/Snoopy.class.php';
        
        $snoopy = new Snoopy;

        // Define the URL to fetch
        $url = "https://servicioswww.anses.gob.ar//ooss2/";//"https://www.anses.gob.ar/consultas/obra-social-codem/"
        
        // Fetch the content
        if ($snoopy->fetch($url)) {
            // If successful, the results are in $snoopy->results
            //echo "<pre>";
            echo str_replace('src="','src="https://servicioswww.anses.gob.ar/', $snoopy->results); // htmlspecialchars to display HTML source safely
           // echo "</pre>";
        
            // You can also access the HTTP response code
    ...
    */
    ...
    <!--
    <iframe src2="https://www.anses.gob.ar/consultas/obra-social-codem/" src2="https://tramites.renaper.gob.ar/mi_ejemplar/" style="width: 100%;"></iframe>
    
    <iframe src="https://servicioswww.anses.gob.ar/ooss2/" src2="https://tramites.renaper.gob.ar/mi_ejemplar/" style="width: 100%;"></iframe>
    -->
    </div>
