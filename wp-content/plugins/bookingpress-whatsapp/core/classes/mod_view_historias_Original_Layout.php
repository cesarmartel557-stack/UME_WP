	...
	<el-row type="flex" class="variapaddding-bpa-mlc-head-wrap" style="border-bottom: 1px solid var(--bpa-gt-gray-300);padding:20px 24px 20px 30px;" >
		<el-col :xs="24" :sm="24" :md="24" :lg="12" :xl="12" class="bpa-mlc-left-heading" style="align-self: flex-start;padding-top: 20px;" >
            <div style="display: flex;flex-direction: column;align-items:start;gap:20px;">
    			<h1 class="bpa-page-heading"><?php esc_html_e( 'Historias Clínicas Ambulatorias', 'bookingpress-appointment-booking' ); ?></h1>
                
                
                <div v-if=""  style="display: flex;align-items: center;min-width: 150px;min-height: 56px;">
                    <h2 v-if="selected_patient" class="bpa-form-label" style="margin-bottom: 0;" >
        				<span class="bpa-form-label" style="color: lightslategrey;text-transform: uppercase;display: flex;flex-wrap:wrap;"> <!-- font-family: 'Inter';font-weight: 300;font-size: 13px; -->
                            ...
                            <div>
                                <span  class="" >{{ (selected_patient.customer_firstname||'') + '&nbsp;' + (selected_patient.customer_lastname||'') }}&nbsp;</span>
                            </div>
                            <div>
            		...
            		<div class="bpa-table-filter is-align-right bp-hc-filter" style="border-bottom: 0;padding-right: 0;">
            			<el-row type="flex" :gutter="16" align="right" class="justify-end">
            				<el-col :xs="24" :sm="12" :md="8" :lg="6" :xl="6" style="min-width: 250px;">
            					<el-input ref="bphc_dni_filter" class="bpa-form-control" v-model="dni_query" placeholder="Ingrese DNI minimo 3 digitos "></el-input>
            				</el-col>
            ...
            <div id="bphc_main_historias_el"  class="el-row main-historias " >
                ...
                <div class="flex w-full histories-listing-header" style="width: 100%;" >
                    ...
                    <div class="flex" style="align-self: flex-end;justify-self: end;padding-right: 10px;min-width: 380px;justify-content: end;">
                        ...
                        <?php $bphc_appointment_id = absint($_GET['appointment']); $bphc_history_popdisplay = absint($_GET['popup-display']);
                ...
                <div class="lista-historias-container bphc_listing_conf-1" style="min-height: 380px;">
                    ...
                    <div class="lista-historias" style="padding-right: 10px;">
                        ...
                        <div v-else v-for="(h, i) in histories" :key="h.id" class="flex history-Slot" >
                            ...
                            <div class="historia-item" @click="bphc_edit_HystoryRecord(h.id, h)">
                                ...
                                <div class="border-l-4 border-primary-400 bg-white rounded shadow-sm p-3 flex"> 
                                    ...
                                    <div class="text-center mr-4 the_history_date">
                                    ...
                                    <div class="bphc_inf_sname">
                                        <div class="flex-col" style="flex-direction: column;gap: 2px;">
                                        
                                            <el-tooltip effect="dark" content="Identificador de historia" placement="top"><el-tag type="info" style="background-color: transparent;">#{{h.id}}</el-tag></el-tooltip>
                                            <el-tooltip effect="dark" :content="h.service_name || 'no especifico especialidad'" placement="bottom" open-delay="300" >
                                    ...
                                    <div class="text-sm bphc_inf_motivo"> 
                                        <div style="display: flex;gap: 4px;flex-wrap: nowrap;">
                                            
                                            <el-tooltip effect="dark" content="Turno relacionado" placement="top">
                                                <el-tag type="info" style="text-align: center;" v-if="h.raw.booking_id">Turno #{{h.raw.booking_id}}</el-tag>
                                                <el-tag type="info" style="text-align: center;" v-else>Sin turno</el-tag>
                                            </el-tooltip>
                                            
                                        </div>
                                        <div>
                                            <div class="motivoConsulta"><span>{{h.general.motivoConsulta|| '' }}</span></div>
                                            <div class="diagnostico"><span>{{h.general.diagnostico|| '' }}</span></div>
                                        </div>
                                    </div>
                                    <div class="text-xs text-gray-500 bphc_inf_right">
                                        <span class="the_history_time">
                                            <span class="material-icons-round">watch_later</span>
                                            {{ (moment((h.time||''), 'HH').format('hh:mm A'))!='invalid_date'? (moment((h.time||''), 'HH').format('hh:mm A')) :''}}
                                        </span>
                                        
                                        <span @click.native.prevent="bphc_edit_HystoryRecord(h.id, h)" class="bphc-action-buttons" style="cursor: pointer;justify-self:right;">
                                        ...
                                        <div class="flex-col">
                                            <el-tag effect="light" style="width: 50px;text-align: center;">{{ h.consultorio || 'consulta' }} </el-tag>
                                        </div>
                                ...
                                <div class="border-l-4 border-primary-400 bg-white rounded shadow-sm p-3 flex">
                                    ...
                                    <div style="width: 100%;display: flex; flex-direction: column;">
                                        ...
                                        <div style="display: flex; flex-direction: row; gap: 4px; width: 100%;justify-content: space-between;margin-bottom: 4px;">
                                            ...
                                            <div class="text-xs text-gray-500 bphc_inf_right" style="min-width: 106px;align-items: center;/*align-self: flex-start;*/">
                                                <span class="the_history_time">
                                                    <span class="material-icons-round">watch_later</span>
                                                    {{ (moment((h.time||''), 'HH').format('hh:mm A'))!='invalid_date'? (moment((h.time||''), 'HH').format('hh:mm A')) :''}}
                                                </span>
                                        ...
                                        <div style="min-height: 50px;display: flex; flex-direction: row; gap: 4px; width: 100%;">
                                            <div style="display: flex; flex-direction: column; gap: 4px;padding: 10px;width: min(300px, 40%);min-width: 100px;">
                                                <span class="text-sm text-gray-500">{{ (selected_patient.customer_firstname||'') + '&nbsp;' + (selected_patient.customer_lastname||'') }}</span>
                                                <span class="text-sm text-gray-500">{{ h.staff_member_name || '' }}</span>
                                            </div>
                                            <div style="background: whitesmoke;display: flex;flex-direction: column;gap: 4px;width: 100%;padding: 10px;/* flex: 1 1 100%; */">
                                                <div class="motivoConsulta" style="max-width: 100%;"><span>{{h.general.motivoConsulta|| '' }}</span></div>
                                                <div class="diagnostico" style="max-width: 100%;"><span>{{h.general.diagnostico|| '' }}</span></div>
                                            </div>
                                        </div>
...
<!-- REGISTRO DE NUEVA HISTORIA custom-class="bpa-dialog bpa-dialog--fullscreen bpa-dialog--history-modal bpa--is-page-non-scrollable-mob" -->

<el-dialog id="bphc_history_add_modal" fullscreen=true modal-append-to-body=false custom-class="bpa-dialog bpa-dialog--fullscreen bpa-dialog--history-modal bpa--is-page-non-scrollable-mob" :visible.sync="bp_hc_ConsultationModal" :before-close="closeCustomerModal"  :close-on-press-escape="close_modal_on_esc">
