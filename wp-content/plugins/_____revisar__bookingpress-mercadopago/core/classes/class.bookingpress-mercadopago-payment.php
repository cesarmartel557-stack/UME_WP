...
$f = fopen(__DIR__ . '/'.'prefes'.'.txt', 'a');
   if( $f ){
    fwrite($f, "\n\n bookingpress_create_preference_body_params: ".print_r($bookingpress_create_preference_body_params, true)."\n" );
    fclose($f); 
   }
                $bookingpress_create_preference_url = "https://api.mercadopago.com/checkout/preferences";
                $bookingpress_created_preference_response = wp_remote_request($bookingpress_create_preference_url, $bookingpress_create_preference_body_params);
        ...
        function bookingpress_payment_gateway_data(){
            ...
            if ( ! empty( $_REQUEST['bookingpress-listener'] ) && ($_REQUEST['bookingpress-listener'] == "bpa_pro_mercadopago_url") ) {
                $this->bookingpress_init_mercadopago();
$f = fopen(__DIR__ . '/'.'noty_post'.'.txt', 'a');
   if( $f ){
    fwrite($f, "\n\n noty: ".print_r($_POST, true)."\n" );
    fclose($f); 
   }
$f = fopen(__DIR__ . '/'.'noty_req'.'.txt', 'a');
   if( $f ){
    fwrite($f, "\n\n noty: ".print_r($_REQUEST, true)."\n" );
    fclose($f); 
   }
$f = fopen(__DIR__ . '/'.'noty_input'.'.txt', 'a');
   if( $f ){
    fwrite($f, "\n\n noty: ".print_r(file_get_contents('php://input'), true)."\n" );
    fclose($f); 
   }
                        ...
                        if(!empty($bookingpress_verified_payment_res_arr['status']) && ($bookingpress_verified_payment_res_arr['status'] == "approved"))
                        {
                            ...
                            if(!empty($bookingpress_payment_metadata)) {
                                ...
                                if(!empty($bookingpress_entry_id)){
                                    ...
                                    ##CODE ADD 
                                    $bookingpress_webhook_data = $bookingpress_verified_payment_res_arr;
                                    
                                    $f = fopen(__DIR__ . '/'.'dato_pago'.'.txt', 'a');
   if( $f ){
    fwrite($f, "\n\n res: ".print_r($bookingpress_webhook_data, true)."\n" );
    fclose($f); 
   }
   //$bookingpress_is_cart = 0;                                 
                                    ...
                                    $payment_log_id = $bookingpress_pro_payment_gateways->bookingpress_confirm_booking( $bookingpress_entry_id, $bookingpress_webhook_data, '1', 'id','transaction_amount', 1, $bookingpress_is_cart);
                                    //echo "CONFIRMADO..................................$payment_log_id";
                                }
                            }
                        }
                    }
